object Form1: TForm1
  Left = 3
  Top = 3
  ActiveControl = StatBox
  BorderStyle = bsToolWindow
  Caption = 'DirScan'
  ClientHeight = 290
  ClientWidth = 378
  Color = 15724527
  Ctl3D = False
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -9
  Font.Name = 'Tahoma'
  Font.Style = [fsBold]
  Menu = Menu
  OldCreateOrder = False
  Position = poDesktopCenter
  PixelsPerInch = 96
  TextHeight = 11
  object PageControl: TPageControl
    Left = 8
    Top = 8
    Width = 361
    Height = 217
    ActivePage = ListTab
    TabOrder = 0
    object ListTab: TTabSheet
      Caption = 'LIST'
      object Memo1: TMemo
        Left = 3
        Top = 3
        Width = 344
        Height = 187
        Color = 11796479
        Enabled = False
        Lines.Strings = (
          '-admin'
          '.bash_history'
          '.bash_profile'
          '.error_log'
          '.ftpquota'
          '.history'
          '.htaccess'
          '.htconfig'
          '.htpasswd'
          '.mysql_history'
          '.passwd'
          '.tools/'
          '/.bash_history'
          '/.bash_profile'
          '/.htaccess'
          '/.htaccess~'
          '/.htpasswd'
          '/.htpasswd~'
          '/.mysql_history'
          '/.passwd'
          '/0.php'
          '/000/'
          '/001/'
          '/002/'
          '/1.php'
          '/1/'
          '/123.php'
          '/123/'
          '/123123/'
          '/123321/'
          '/12345/'
          '/123456/'
          '/2.php'
          '/2/'
          '/2000/'
          '/2001/'
          '/2002/'
          '/2003/'
          '/2004/'
          '/2005/'
          '/2006/'
          '/2007/'
          '/2008/'
          '/2009/'
          '/2010/'
          '/2011/'
          '/2012/'
          '/3.php'
          '/3/'
          '/4.php'
          '/4/'
          '/4admin/'
          '/4dm1n/'
          '/4dmin/'
          '/5.php'
          '/5/'
          '/6.php'
          '/6/'
          '/7.php'
          '/7/'
          '/8.php'
          '/8/'
          '/9.php'
          '/9/'
          '/AT-admin.cgi'
          '/AdminControlPanel/'
          '/AdminWeb/'
          '/AnyForm2'
          '/CS/'
          '/CVS/'
          '/CertControl/'
          '/CertEnroll/'
          '/ClientAdmin/'
          '/Common'
          '/Default.php'
          '/Fuel/'
          '/GW5/GWWEB.EXE'
          '/GXApp/'
          '/HB/'
          '/InformationSystems/'
          '/MSMQ/'
          '/NSearch/'
          '/PBSData/'
          '/PBServer/'
          '/README.txt'
          '/REC-html40/'
          '/SQL/'
          '/StudentGroups/'
          '/TABadmin/'
          '/TR/'
          '/Temporary/'
          '/ToDo/'
          '/Update.sql'
          '/UserAccess/'
          '/WINDMAIL.EXE?%20-n%20c:'
          '\boot.ini%'
          '/WebBank/'
          '/WebShop/'
          '/___admin/'
          '/__admin/'
          '/_admin/'
          '/_back/'
          '/_backup/'
          '/_dump/'
          '/_errors/'
          '/_install.php'
          '/_install/'
          '/_mem_bin/'
          '/_pages/'
          '/_private/'
          '/_scripts/'
          '/_sess/'
          '/_sessions/'
          '/_startup.php'
          '/_system/'
          '/_vti_bin/'
          '/_vti_bot/'
          '/_vti_cnf/'
          '/_vti_inf.html'
          '/_vti_log/'
          '/_vti_pvt/'
          '/_vti_script/'
          '/_vti_shm/'
          '/_vti_txt/'
          '/a-d-m-i-n/'
          '/a.php'
          '/a/'
          '/a_d_m_i_n/'
          '/aaa/'
          '/about/'
          '/academia/'
          '/acart/'
          '/acc/'
          '/acces/'
          '/access.log'
          '/access/'
          '/access_log'
          '/account/'
          '/accounts/'
          '/acess/'
          '/achievo/'
          '/active/'
          '/ad/'
          '/add.php'
          '/add/'
          '/addons/'
          '/address/'
          '/adm.php'
          '/adm/'
          '/adm1/'
          '/adm1n/'
          '/admen/'
          '/admi/'
          '/admin-a/'
          '/admin-login.php'
          '/admin.pages/'
          '/admin.php'
          '/admin/'
          '/admin/mysql/'
          '/admin/phpmyadmin/'
          '/admin/pma/'
          '/admin1/'
          '/admin2/'
          '/admin3/'
          '/admin777/'
          '/admin_reply/'
          '/admincp/'
          '/admindir/'
          '/adminenter/'
          '/administration/'
          '/administrator.php'
          '/administrator/'
          '/administrators/'
          '/adminka/'
          '/adminlogin.asp'
          '/adminlogin.html'
          '/adminlogin.php'
          '/adminlogin/'
          '/adminpage/'
          '/adminpages/'
          '/adminpanel/'
          '/adminpatch/'
          '/admins/'
          '/adminsignup/'
          '/admintool/'
          '/admintools/'
          '/ads/'
          '/adsensepackages/'
          '/aedit/'
          '/aglimpse'
          '/ajax/'
          '/alias/'
          '/aliases/'
          '/alibaba.pl'
          '/all/'
          '/allow/'
          '/alpha/'
          '/amanager/'
          '/apache/'
          '/apanel/'
          '/app-admin/'
          '/application/'
          '/applications/'
          '/arc/'
          '/archive/'
          '/archives/'
          '/args.cmd'
          '/article/'
          '/articles/'
          '/ashop/'
          '/assets/'
          '/audio/'
          '/audit/'
          '/auth.do'
          '/auth.html'
          '/auth.php'
          '/auth/'
          '/av/'
          '/ava/'
          '/avatar/'
          '/avatars/'
          '/awstats/'
          '/b/'
          '/back/'
          '/backup.sql'
          '/backup/'
          '/bad.txt'
          '/ban/'
          '/bank/'
          '/banner/'
          '/base/'
          '/basic/'
          '/beta/'
          '/billpay/'
          '/bin/'
          '/block/'
          '/blog/'
          '/bnbform.cgi'
          '/boardroom/'
          '/book/'
          '/boot/'
          '/boy/'
          '/boys/'
          '/browser/'
          '/build/'
          '/c.php'
          '/c/'
          '/c99.php'
          '/c99mad.php'
          '/c99madshell.php'
          '/cache/'
          '/caches/'
          '/campas'
          '/card/'
          '/cards/'
          '/cash/'
          '/casino/'
          '/catalog/'
          '/cbi-bin/'
          '/cc/'
          '/ccbill/'
          '/cdrom/'
          '/ceditor.htm'
          '/ceditor.php'
          '/central/'
          '/certsrv/'
          '/cgi-914/'
          '/cgi-915/'
          '/cgi-auth/'
          '/cgi-bin/'
          '/cgi-bin/AT-admin.cgi'
          '/cgi-bin/AnyBoard.cgi'
          '/cgi-bin/AnyForm2'
          '/cgi-bin/Count.cgi'
          '/cgi-bin/aglimpse'
          '/cgi-bin/ax-admin.cgi'
          '/cgi-bin/ax.cgi'
          '/cgi-bin/bnbform.cgi'
          '/cgi-bin/cachemgr.cgi'
          '/cgi-bin/campas'
          '/cgi-bin/cgi-lib.pl'
          '/cgi-bin/cgiwrap'
          '/cgi-bin/classifieds.cgi'
          '/cgi-bin/dumpenv.pl'
          '/cgi-bin/edit.pl'
          '/cgi-bin/environ.cgi'
          '/cgi-bin/faxsurvey'
          '/cgi-bin/filemail.cgi'
          '/cgi-bin/files.pl'
          '/cgi-bin/finger'
          '/cgi-bin/gH.cgi'
          '/cgi-bin/glimpse'
          '/cgi-bin/guestbook.cgi'
          '/cgi-bin/guestbook.pl'
          '/cgi-bin/handler'
          '/cgi-bin/htmldocs'
          '/cgi-bin/htmlscript'
          '/cgi-bin/info2www'
          '/cgi-bin/jj'
          '/cgi-bin/login.cgi'
          '/cgi-bin/maillist.cgi'
          '/cgi-bin/man.sh'
          '/cgi-bin/minimal.exe'
          '/cgi-bin/nph-publish'
          '/cgi-bin/nph-test-cgi'
          '/cgi-bin/passwd'
          '/cgi-bin/passwd.txt'
          '/cgi-bin/password'
          '/cgi-bin/password.txt'
          '/cgi-bin/perl.exe'
          '/cgi-bin/perlshop.cgi'
          '/cgi-bin/pfdisplay'
          '/cgi-bin/phf'#9'H'
          '/cgi-bin/phf.cgi'
          '/cgi-bin/php'
          '/cgi-bin/php.cgi'
          '/cgi-bin/phpmylog.phtml'
          '/cgi-bin/query'
          '/cgi-bin/redir.exe'
          '/cgi-bin/rguest.exe'
          '/cgi-bin/rwwwshell.pl'
          '/cgi-bin/stats.prg'
          '/cgi-bin/statsconfig'
          '/cgi-bin/survey.cgi'
          '/cgi-bin/test-cgi'
          '/cgi-bin/textcounter.pl'
          '/cgi-bin/unlg1.1'
          '/cgi-bin/unlg1.2'
          '/cgi-bin/view-source'
          '/cgi-bin/visitor.exe'
          '/cgi-bin/webbbs.cgi'
          '/cgi-bin/webdist.cgi'
          '/cgi-bin/webgais'
          '/cgi-bin/websendmail'
          '/cgi-bin/wguest.exe'
          '/cgi-bin/whois_raw.cgi'
          '/cgi-bin/wrap'
          '/cgi-bin/www-sql'
          '/cgi-bin/wwwboard.cgi'
          '/cgi-bin/wwwboard.pl'
          '/cgi-bin2/'
          '/cgi-csc/'
          '/cgi-dos/'
          '/cgi-dos/args.bat'
          '/cgi-exe/'
          '/cgi-home/'
          '/cgi-lib/'
          '/cgi-local/'
          '/cgi-perl/'
          '/cgi-scripts/'
          '/cgi-shl/'
          '/cgi-shop/'
          '/cgi-src/'
          '/cgi-sys/'
          '/cgi-temp/'
          '/cgi-win/'
          '/cgi-win/uploader.exe'
          '/cgi.cgi/'
          '/cgi/'
          '/cgi_bin/'
          '/cgibin/'
          '/cgis/'
          '/cgiwrap'
          '/change-your-password.pl~'
          '/changlog/'
          '/chat/'
          '/check/'
          '/citrix/'
          '/clas/'
          '/class/'
          '/classes/'
          '/classifieds.cgi'
          '/clickresponder.pl'
          '/client/'
          '/clients/'
          '/closed/'
          '/closeup/'
          '/cms/'
          '/cmsadmin/'
          '/cmsimple/'
          '/code/'
          '/codes/'
          '/com/'
          '/comment/'
          '/common/'
          '/community/'
          '/component/'
          '/components/'
          '/conf/'
          '/conference/'
          '/conferences/'
          '/config.inc'
          '/config.inc.php'
          '/config.inc.php~'
          '/config.inc~'
          '/config.php'
          '/config.php.inc'
          '/config.php.inc~'
          '/config.php~'
          '/config.txt'
          '/config/'
          '/configs/'
          '/configuration.txt'
          '/configuration/'
          '/connect/'
          '/console/'
          '/content/'
          '/control-panel/'
          '/control/'
          '/controllers/'
          '/controlpanel/'
          '/cookie/'
          '/cool/'
          '/count.cgi'
          '/count/'
          '/cp/'
          '/cpadmin/'
          '/cpanel.php'
          '/cpanel/'
          '/cpmoder/'
          '/css/'
          '/customphp/'
          '/cvsweb/'
          '/cybercash/'
          '/d/'
          '/da/'
          '/daemon/'
          '/dat/'
          '/data/'
          '/database/'
          '/databases/'
          '/datas/'
          '/day5datacopier.cgi'
          '/day5datanotifier.cgi'
          '/db/'
          '/dbadmin/'
          '/dbase/'
          '/dblog/'
          '/default/'
          '/delivery/'
          '/demo/'
          '/demoadmin/'
          '/deny/'
          '/design/'
          '/dev/'
          '/development/'
          '/dir/'
          '/directory/'
          '/dirs/'
          '/disk/'
          '/disks/'
          '/dist/'
          '/distr/'
          '/distrib/'
          '/distribution/'
          '/distributions/'
          '/dmr/'
          '/doc-html/'
          '/doc/'
          '/docmanager/'
          '/docs/'
          '/docs/phpMyAdmin/'
          '/docs/phpMyAdmin/db_stats.asp'
          '/docs/phpMyAdmin/db_stats.php'
          '/docs/phpMyAdmin/user_details.asp'
          '/docs/phpMyAdmin/user_details.php'
          '/document/'
          '/documents/'
          '/domain/'
          '/down/'
          '/download/'
          '/downloads/'
          '/dtd/'
          '/dump.sql'
          '/dump/'
          '/dumpenv.pl'
          '/dumper.php'
          '/dumper/'
          '/e/'
          '/edit.php'
          '/edit.pl'
          '/editor/'
          '/email.txt'
          '/email/'
          '/emails/'
          '/employer/'
          '/enter/'
          '/environ.cgi'
          '/epoch/'
          '/error.log'
          '/error/'
          '/error_log'
          '/errors/'
          '/etc/'
          '/example/'
          '/examples/'
          '/exchange.log'
          '/exe/'
          '/exp/'
          '/export/'
          '/extr/'
          '/f/'
          '/faq/'
          '/faxsurvey'
          '/fcgi-bin/'
          '/field/'
          '/file/'
          '/fileManager.php'
          '/filemail.pl'
          '/filemanager/'
          '/files/'
          '/films/'
          '/find/'
          '/finger'
          '/finger?@localhost'
          '/flash/'
          '/folder/'
          '/form.cgi'
          '/formmail.pl'
          '/forum/'
          '/forums/'
          '/forwrite/'
          '/foto/'
          '/fotos/'
          '/fpexplore.exe'
          '/ftp/'
          '/fuck/'
          '/fuckyou/'
          '/func/'
          '/funcctions/'
          '/function/'
          '/g/'
          '/gadmin/'
          '/gal/'
          '/gall/'
          '/gallery/'
          '/general/'
          '/get/'
          '/get32.exe'
          '/girl/'
          '/girls/'
          '/global/'
          '/god/'
          '/gold/'
          '/good.txt'
          '/gpanel/'
          '/group/'
          '/groups/'
          '/guest/'
          '/guestbook.cgi'
          '/guestbook/'
          '/guestbooks/'
          '/gui/'
          '/h/'
          '/handler'
          '/handler.cgi'
          '/hello.bat'
          '/help/'
          '/hide/'
          '/history/'
          '/home/'
          '/host/'
          '/hosts/'
          '/htaccess.txt'
          '/htbin/'
          '/htdocs/'
          '/html/'
          '/htmlscript'
          '/htsearch'
          '/i/'
          '/ibank/'
          '/ibill/'
          '/icon/'
          '/icons/'
          '/icq/'
          '/id/'
          '/iisadmin/'
          '/iisadmpwd/'
          '/iishelp/'
          '/iissamples/'
          '/im/'
          '/image/'
          '/images/'
          '/images2/'
          '/img/'
          '/imgedit/'
          '/imp/'
          '/import/'
          '/in/'
          '/inc/'
          '/includ/'
          '/include/'
          '/includes/'
          '/incs/'
          '/index.html'
          '/index/'
          '/index1.html'
          '/index1.php'
          '/index2.html'
          '/index2.php'
          '/index3.php'
          '/index4.php'
          '/index5.php'
          '/info.php'
          '/info/'
          '/info2www'
          '/information/'
          '/infosrch.cgi'
          '/ini/'
          '/input.bat'
          '/input2.bat'
          '/install.php'
          '/install/'
          '/installation/'
          '/international/'
          '/ip/'
          '/irc/'
          '/isapi/'
          '/j/'
          '/java/'
          '/javascript/'
          '/jdbc/'
          '/jj'
          '/js/'
          '/jscript/'
          '/jscripts/'
          '/jserv/'
          '/jsp/'
          '/k/'
          '/key.php'
          '/key/'
          '/keys/'
          '/l/'
          '/labs/'
          '/lang/'
          '/language/'
          '/languages/'
          '/lib/'
          '/librarie/'
          '/libraries/'
          '/library/'
          '/link/'
          '/linkadmin/'
          '/links/'
          '/list/'
          '/listingadmin/'
          '/listings/'
          '/lite/'
          '/lng/'
          '/load/'
          '/loaded/'
          '/loader/'
          '/loadpage.cgi'
          '/loads/'
          '/local/'
          '/log-in/'
          '/log/'
          '/log_in/'
          '/login-admin.php'
          '/login.php'
          '/login/'
          '/loginadmin/'
          '/logins/'
          '/logon/'
          '/logos/'
          '/logout/'
          '/logs-a/'
          '/logs/'
          '/look/'
          '/m.php'
          '/m/'
          '/mail.php'
          '/mail/'
          '/mailer/'
          '/mailit.pl'
          '/maillist.pl'
          '/mailroot/'
          '/mails/'
          '/makefile/'
          '/man.sh'
          '/manage/'
          '/manager.php'
          '/manager/'
          '/master/'
          '/mastergate/'
          '/mb/'
          '/me/'
          '/media/'
          '/member.php'
          '/member/'
          '/members.php'
          '/members/'
          '/mini/'
          '/mirago/'
          '/misc/'
          '/mmyyaaddmmiinn'
          '/mobile/'
          '/mod/'
          '/moder/'
          '/moderation/'
          '/moderator/'
          '/modercp/'
          '/modul/'
          '/modules/'
          '/money/'
          '/mp3/'
          '/mpcgi/'
          '/mpej/'
          '/mrtg/'
          '/msadc/'
          '/mssql/'
          '/multisite/'
          '/music/'
          '/mw/'
          '/my/'
          '/myadmin/'
          '/myphpadmin/'
          '/mysql-admin/'
          '/mysql.php'
          '/mysql.sql'
          '/mysql/'
          '/mysqladmin/'
          '/n/'
          '/n031337f1l3'
          '/net/'
          '/netbilling/'
          '/netcat/'
          '/network/'
          '/new/'
          '/news/'
          '/note/'
          '/nph-publish'
          '/nph-test-cgi'
          '/o/'
          '/odbc/'
          '/oetaki/'
          '/office/'
          '/old/'
          '/open/'
          '/operator/'
          '/order/'
          '/orders/'
          '/other/'
          '/out/'
          '/ows-bin/'
          '/p/'
          '/page/'
          '/pages/'
          '/panel.html'
          '/panel.php'
          '/panel/'
          '/pars/'
          '/parser.php'
          '/parser/'
          '/pas/'
          '/pass/'
          '/passport/'
          '/passwd.adjunct/'
          '/passwd/'
          '/password/'
          '/passwords/'
          '/path/'
          '/paths/'
          '/perl.exe'
          '/perl/'
          '/perl5/'
          '/perlshop.cgi'
          '/pfdisplay.cgi'
          '/pharma/'
          '/pharmacy/'
          '/phf'
          '/phf.cgi'
          '/phf.pp'
          '/phone/'
          '/phones/'
          '/photo/'
          '/photos/'
          '/php'
          '/php-adminpanel/'
          '/php-cgi'
          '/php.cgi'
          '/php.ini'
          '/php.php'
          '/php/'
          '/phpBB/'
          '/phpMyA'
          '/phpMyA/'
          '/phpMyAdmi'
          '/phpMyAdmi/'
          '/phpMyAdmin-2'
          '/phpMyAdmin-2.10.0'
          '/phpMyAdmin-2.10.0/'
          '/phpMyAdmin-2.10.1/'
          '/phpMyAdmin-2.10.2/'
          '/phpMyAdmin-2.10.3/'
          '/phpMyAdmin-2.11.0/'
          '/phpMyAdmin-2.11.1/'
          '/phpMyAdmin-2.11.10/'
          '/phpMyAdmin-2.11.2/'
          '/phpMyAdmin-2.11.3/'
          '/phpMyAdmin-2.11.4/'
          '/phpMyAdmin-2.11.5/'
          '/phpMyAdmin-2.11.6/'
          '/phpMyAdmin-2.11.7/'
          '/phpMyAdmin-2.11.8.1/'
          '/phpMyAdmin-2.11.9/'
          '/phpMyAdmin-2.2.3/'
          '/phpMyAdmin-2.2.6/'
          '/phpMyAdmin-2.3.0'
          '/phpMyAdmin-2.3.1'
          '/phpMyAdmin-2.3.2'
          '/phpMyAdmin-2.3.3'
          '/phpMyAdmin-2.3.4'
          '/phpMyAdmin-2.3.5'
          '/phpMyAdmin-2.3.6'
          '/phpMyAdmin-2.3.7'
          '/phpMyAdmin-2.3.8'
          '/phpMyAdmin-2.3.9'
          '/phpMyAdmin-2.4.0'
          '/phpMyAdmin-2.4.1'
          '/phpMyAdmin-2.4.2'
          '/phpMyAdmin-2.4.3'
          '/phpMyAdmin-2.4.4'
          '/phpMyAdmin-2.4.5'
          '/phpMyAdmin-2.4.6'
          '/phpMyAdmin-2.4.7'
          '/phpMyAdmin-2.4.8'
          '/phpMyAdmin-2.4.9'
          '/phpMyAdmin-2.5.0'
          '/phpMyAdmin-2.5.1'
          '/phpMyAdmin-2.5.1/'
          '/phpMyAdmin-2.5.2'
          '/phpMyAdmin-2.5.3'
          '/phpMyAdmin-2.5.4'
          '/phpMyAdmin-2.5.4/'
          '/phpMyAdmin-2.5.5'
          '/phpMyAdmin-2.5.6'
          '/phpMyAdmin-2.5.6/'
          '/phpMyAdmin-2.5.7'
          '/phpMyAdmin-2.5.8'
          '/phpMyAdmin-2.5.9'
          '/phpMyAdmin-2.6.0'
          '/phpMyAdmin-2.6.0-pl1/'
          '/phpMyAdmin-2.6.0/'
          '/phpMyAdmin-2.6.1'
          '/phpMyAdmin-2.6.2'
          '/phpMyAdmin-2.6.2-rc1/'
          '/phpMyAdmin-2.6.3'
          '/phpMyAdmin-2.6.3-pl1/'
          '/phpMyAdmin-2.6.3-rc1/'
          '/phpMyAdmin-2.6.3/'
          '/phpMyAdmin-2.6.4'
          '/phpMyAdmin-2.6.5'
          '/phpMyAdmin-2.6.6'
          '/phpMyAdmin-2.6.7'
          '/phpMyAdmin-2.6.8'
          '/phpMyAdmin-2.6.9'
          '/phpMyAdmin-2.7.0'
          '/phpMyAdmin-2.7.1'
          '/phpMyAdmin-2.7.2'
          '/phpMyAdmin-2.7.3'
          '/phpMyAdmin-2.7.4'
          '/phpMyAdmin-2.7.5'
          '/phpMyAdmin-2.7.6'
          '/phpMyAdmin-2.7.7'
          '/phpMyAdmin-2.7.8'
          '/phpMyAdmin-2.7.9'
          '/phpMyAdmin-2.8.1'
          '/phpMyAdmin-2.8.2'
          '/phpMyAdmin-2.8.3'
          '/phpMyAdmin-2.8.4'
          '/phpMyAdmin-2.8.5'
          '/phpMyAdmin-2.8.6'
          '/phpMyAdmin-2.8.7'
          '/phpMyAdmin-2.8.8'
          '/phpMyAdmin-2.8.9'
          '/phpMyAdmin-2.9.1'
          '/phpMyAdmin-2.9.2'
          '/phpMyAdmin-2/'
          '/phpMyAdmin-3'
          '/phpMyAdmin-3.0.0/'
          '/phpMyAdmin-3.0.1/'
          '/phpMyAdmin-3.1.0/'
          '/phpMyAdmin-3.1.1/'
          '/phpMyAdmin-3.1.2/'
          '/phpMyAdmin-3.1.3/'
          '/phpMyAdmin-3.1.4/'
          '/phpMyAdmin-3.1.5/'
          '/phpMyAdmin-3.2.0/'
          '/phpMyAdmin-3.2.1/'
          '/phpMyAdmin-3.2.2/'
          '/phpMyAdmin-3.2.3/'
          '/phpMyAdmin-3.2.4/'
          '/phpMyAdmin-3.2.5/'
          '/phpMyAdmin-3.3.0/'
          '/phpMyAdmin-3.3.1/'
          '/phpMyAdmin-3.3.2-rc1/'
          '/phpMyAdmin-3.3.2/'
          '/phpMyAdmin-3.3.3-rc1/'
          '/phpMyAdmin-3.3.3/'
          '/phpMyAdmin-3.3.4-rc1/'
          '/phpMyAdmin-3.3.4/'
          '/phpMyAdmin-3/'
          '/phpMyAdmin-4'
          '/phpMyAdmin-4/'
          '/phpMyAdmin/db_stats.php'
          '/phpMyAdmin/export.php?'
          'what=../../../../../../etc/passwd%00'
          '/phpMyAdmin/sql.php'
          '/phpMyAdmin/sql.php3'
          '/phpMyAdmin/sql.php?'
          'goto=/etc/hosts&btnDrop=No'
          '/phpMyAdmin/tbl_copy.php'
          '/phpMyAdmin/tbl_copy.php?'
          'db=test&table=haxor&new_name=test.ha'
          'xor'
          '2&strCopyTableOK="".passthru'
          '('#39'/bin/ls'#39')."""'
          '/phpMyAdmin/tbl_copy.php?'
          'strCopyTableOK=.passthru('#39'/bin/ls'#39').'
          '/phpMyAdmin/tbl_replace.php?'
          'db=test&table=ess&goto=/etc/hosts'
          '/phpMyAdmin/user_details.php'
          '/phpMyAdmin1'
          '/phpMyAdmin1/'
          '/phpMyAdmin2'
          '/phpMyAds.asp'
          '/phpMyAds.php'
          '/phpMyAds/'
          '/phpinfo.php'
          '/phpm/'
          '/phpmy'
          '/phpmy/'
          '/phpmyad'
          '/phpmyad-sys'
          '/phpmyad-sys/'
          '/phpmyad/'
          '/phpmyadm'
          '/phpmyadmin'
          '/phpmyadmin.php'
          '/phpmyadmin.sql'
          '/phpmyadmin/'
          '/phpmyadmin/libraries/grab_globals.lib.p'
          'hp'
          '/phpmyadmin2/'
          '/phpnuke/'
          '/phpproject/'
          '/phpsess/'
          '/picture/'
          '/picture_library/'
          '/pictures/'
          '/plesk-stat/'
          '/plugin/'
          '/plugins/'
          '/plusmail'
          '/pma.php'
          '/pma/'
          '/porn/'
          '/port/'
          '/post/'
          '/postadmin/'
          '/postgres/'
          '/postnuke/'
          '/pphhppmmyyaaddmmiin'
          '/printer/'
          '/printers/'
          '/priv/'
          '/private/'
          '/product/'
          '/products/'
          '/profile/'
          '/profiles/'
          '/program/'
          '/programfiles/'
          '/psm/'
          '/pub/'
          '/public/'
          '/pwtn_application/'
          '/q.php'
          '/q/'
          '/qaz/'
          '/qazwsx/'
          '/qqq/'
          '/query'
          '/qwe.php'
          '/qwe/'
          '/qwerty/'
          '/r/'
          '/r57.php'
          '/radmin/'
          '/radmin/docs/phpMyAdmin/'
          '/radmin/docs/phpMyAdmin/db_stats.asp'
          '/radmin/docs/phpMyAdmin/db_stats.php'
          '/rdp/'
          '/read/'
          '/reg/'
          '/register/'
          '/registration/'
          '/registrations/'
          '/remote/'
          '/rep/'
          '/report/'
          '/reports.txt'
          '/reports/'
          '/res/'
          '/resource/'
          '/resources/'
          '/responder.cgi'
          '/restricted/'
          '/result/'
          '/results/'
          '/rguest.exe'
          '/robot.txt'
          '/robots.txt'
          '/root/'
          '/rpc/'
          '/rss/'
          '/rwwwshell.pl'
          '/s.php'
          '/s/'
          '/sa/'
          '/sample/'
          '/sample1/'
          '/sample2/'
          '/samples/'
          '/save/'
          '/scores/'
          '/scr/'
          '/script/'
          '/scripts/'
          '/scripts/phpmylog.phtml'
          '/sdf/'
          '/search.cgi'
          '/search/'
          '/secret/'
          '/secrets/'
          '/secure/'
          '/security/'
          '/send/'
          '/sendform.cgi'
          '/servadmin/'
          '/server-status/'
          '/server/'
          '/servers/'
          '/service/'
          '/services/'
          '/sess/'
          '/session/'
          '/sessions/'
          '/set/'
          '/sets/'
          '/setting/'
          '/settings/'
          '/setup.info.php'
          '/setup.php'
          '/setup/'
          '/she11.php'
          '/shell.php'
          '/shelll.php'
          '/shop/'
          '/shopping/'
          '/shutdown/'
          '/sign-up.html'
          '/sign-up.php'
          '/sign-up/'
          '/sign_up.html'
          '/sign_up.php'
          '/sign_up/'
          '/signup.html'
          '/signup.php'
          '/signup/'
          '/signupadmin/'
          '/silver/'
          '/site-admin/'
          '/site/'
          '/siteadmin/'
          '/sites/'
          '/skin/'
          '/slave/'
          '/small/'
          '/smart/'
          '/smie/'
          '/soft/'
          '/sojourn.cgi'
          '/sound/'
          '/source/'
          '/sources/'
          '/sql.php'
          '/sql.sql'
          '/sqladmin/'
          '/src/'
          '/ssi/'
          '/ssl/'
          '/startup.php'
          '/stat/'
          '/statistics/'
          '/stats/'
          '/status/'
          '/store/'
          '/students/'
          '/style/'
          '/survey.cgi'
          '/sys/'
          '/sysadmin/'
          '/sysinfo/'
          '/syslog/'
          '/sysman/'
          '/sysop/'
          '/system/'
          '/system32/'
          '/systemadm/'
          '/systemadmin/'
          '/systemadministration/'
          '/t/'
          '/tag/'
          '/tags/'
          '/temp/'
          '/template/'
          '/templates/'
          '/test-cgi'
          '/test-cgi/'
          '/test.bat'
          '/test/'
          '/test/test.cgi'
          '/test12/'
          '/tests/'
          '/testtest/'
          '/textcounter.pl'
          '/themes/'
          '/thumbs/'
          '/tigvote.cgi'
          '/tmp.php'
          '/tmp/'
          '/to_adm/'
          '/tool/'
          '/tools/'
          '/top/'
          '/total/'
          '/trash/'
          '/tree/'
          '/treeadmin.php'
          '/trust/'
          '/tshirt/'
          '/tst.bat'
          '/tsweb/'
          '/u/'
          '/uddi/'
          '/uddipublic/'
          '/unix/'
          '/unlg1.1'
          '/up/'
          '/update/'
          '/updates/'
          '/upload.pl'
          '/upload/'
          '/uploaded/'
          '/uploader.exe'
          '/uploads/'
          '/ups/'
          '/usage/'
          '/user.php'
          '/user.txt'
          '/user/'
          '/userbase/'
          '/userfile/'
          '/userfiles/'
          '/users.php'
          '/users.txt'
          '/users/'
          '/usr/'
          '/util/'
          '/utils/'
          '/uucp/'
          '/v.php'
          '/v/'
          '/v2/'
          '/var/'
          '/vbulletin/'
          '/vendor/'
          '/version/'
          '/video/'
          '/view-source'
          '/view/'
          '/views/'
          '/visadmin.exe?user=guest'
          '/vsadmin/'
          '/vv/'
          '/vvc/'
          '/vvv/'
          '/w/'
          '/web/'
          '/web/phpMyAdmin/'
          '/web2/'
          '/web20/'
          '/webaccess/'
          '/webadmin/'
          '/webboard/'
          '/webcash/'
          '/webcgi/'
          '/webdist.cgi'
          '/webgais'
          '/weblog/'
          '/websendmail'
          '/website/'
          '/webstat/'
          '/webutil.pl'
          '/webutils.pl'
          '/webwho.pl'
          '/wguest.exe'
          '/whois_raw.cgi'
          '/willopen/'
          '/wm/'
          '/wp-admin/'
          '/wp-login.php'
          '/wrap.cgi'
          '/write/'
          '/ww/'
          '/www-root/'
          '/www-sql'
          '/www/'
          '/wwwadmin.pl'
          '/wwwboard.pl'
          '/wwwboard/eleeteleet'
          '/wwwjoin/'
          '/wwwlog/'
          '/wwwroot/'
          '/x.php'
          '/x/'
          '/xml/'
          '/xp.php'
          '/xsl_admin/'
          '/xx.php'
          '/xxx/'
          '/y/'
          '/z.php'
          '/z/'
          '/zip/'
          '/zz/'
          '/zzz/'
          '/~apache'
          '/~nobody'
          '/~root'
          '/~tmp/'
          '0'
          '0.php'
          '0/'
          '00/'
          '000/'
          '0000/'
          '00000/'
          '000000/'
          '0000000/'
          '00000000/'
          '000000000/'
          '0000000000/'
          '000001/'
          '000002/'
          '000003/'
          '000004/'
          '000005/'
          '000006/'
          '000007/'
          '000008/'
          '000009/'
          '00001'
          '00001/'
          '00002/'
          '00003/'
          '00004/'
          '00005/'
          '00006/'
          '00007/'
          '00008/'
          '00009/'
          '0001'
          '0001/'
          '0002/'
          '0003/'
          '0004/'
          '0005/'
          '0006/'
          '0007/'
          '0008/'
          '0009/'
          '001'
          '001/'
          '002/'
          '003/'
          '004/'
          '005/'
          '006/'
          '007'
          '007/'
          '008/'
          '009/'
          '01/'
          '02/'
          '03/'
          '04/'
          '05/'
          '06/'
          '07/'
          '08/'
          '09/'
          '0admin/'
          '0day'
          '0day/'
          '0manager/'
          '0test/'
          '1'
          '1.php'
          '1/'
          '10/'
          '100/'
          '1000/'
          '10000/'
          '100000/'
          '11'
          '11/'
          '111'
          '111/'
          '1111'
          '1111/'
          '11111/'
          '111111/'
          '123'
          '123/'
          '123123/'
          '123123123/'
          '123123123123/'
          '123321/'
          '123321123321/'
          '123settle/'
          '1337'
          '1337/'
          '1test/'
          '2'
          '2.php'
          '2/'
          '20/'
          '200/'
          '2000/'
          '20000/'
          '200000/'
          '2000000/'
          '2001/'
          '2002/'
          '2003/'
          '2004/'
          '2005/'
          '2006/'
          '2007/'
          '2008/'
          '2009/'
          '2010/'
          '22/'
          '222/'
          '2222/'
          '22222/'
          '222222/'
          '2999/'
          '2test/'
          '3'
          '3.php'
          '3/'
          '30/'
          '300/'
          '3000/'
          '30000/'
          '300000/'
          '3000000/'
          '322alt/'
          '322cms/'
          '322fpdf/'
          '322image/'
          '322include/'
          '33/'
          '333/'
          '3333/'
          '33333/'
          '333333/'
          '3test/'
          '4'
          '4.php'
          '4/'
          '40/'
          '400/'
          '4000/'
          '40000/'
          '400000/'
          '4000000/'
          '44/'
          '444/'
          '4444/'
          '44444/'
          '444444/'
          '4images_users'
          '4test/'
          '5'
          '5.php'
          '5/'
          '50/'
          '500/'
          '5000/'
          '50000/'
          '500000/'
          '5000000/'
          '55/'
          '555/'
          '5555/'
          '55555/'
          '555555/'
          '5test/'
          '6'
          '6.php'
          '6/'
          '60/'
          '600/'
          '6000/'
          '60000/'
          '600000/'
          '6000000/'
          '606/'
          '66/'
          '666/'
          '6666/'
          '66666/'
          '666666/'
          '6test/'
          '7'
          '7.php'
          '7/'
          '70/'
          '700/'
          '7000/'
          '70000/'
          '700000/'
          '7000000/'
          '77/'
          '777'
          '777/'
          '7777/'
          '77777/'
          '777777/'
          '7test/'
          '8'
          '8.php'
          '8/'
          '80/'
          '800/'
          '8000/'
          '80000/'
          '800000/'
          '8000000/'
          '88/'
          '888/'
          '8888/'
          '88888/'
          '888888/'
          '8test/'
          '9'
          '9.php'
          '9/'
          '90/'
          '900/'
          '9000/'
          '90000/'
          '900000/'
          '9000000/'
          '911/'
          '911911/'
          '911text/'
          '99/'
          '999/'
          '9999/'
          '99999/'
          '999999/'
          '9test/'
          '@dmin/'
          '@index.php'
          'ACT_INFO'
          'ASPSamp/'
          'ActiveDataFeed'
          'AddConfirmation/'
          'AddToQueue/'
          'Address/'
          'Admin123/'
          'AdminTools/'
          'AdminWeb/'
          'Admin_/'
          'Admin_dir/'
          'Aemter_01_bis_2002Vergabestelle/'
          'Affichage1'
          'Aipp/'
          'Amministrazione/'
          'BUSINESS/'
          'Benefits/'
          'Berkeley/'
          'BillingActivity/'
          'BillingHistory/'
          'Bs/'
          'CC_info'
          'CC_username'
          'CHANGELOG/'
          'COPYRIGHT.txt'
          'CS/'
          'CSV/'
          'CTH/'
          'CVS'
          'CVS/'
          'CYBERDOCS'
          'CYBERDOCS25'
          'CYBERDOCS31'
          'Cancel/'
          'CatchTheWeb/'
          'Category'
          'CategoryGroup'
          'Catogorie'
          'CertControl/'
          'CertEnroll/'
          'Cfdocs/'
          'Cfide/'
          'Change/'
          'Charge/'
          'ChicksPass'
          'ClickTrack'
          'Common/'
          'Connections/'
          'Country'
          'CountryCodes1'
          'CreditCard/'
          'CustomNav'
          'CustomerService/'
          'DEDICATE/'
          'DVDDistribution/'
          'DataFeedPerformance1'
          'DataFeedShowtag1'
          'DataImport/'
          'Database_Administration/'
          'Default.htm'
          'Diversetest/'
          'Dragon_users'
          'Dx/'
          'EmailSubscription/'
          'EnerPhys/'
          'Event'
          'EventRedirect'
          'Event_Category'
          'Event_backup'
          'Events_new'
          'Excite/'
          'ExistingMember/'
          'Extranet'
          'FCKeditor/'
          'FilterIssues/'
          'Flashtest/'
          'Frames/'
          'FriendPage/'
          'Friends/'
          'Ftp-old/'
          'GANDALF/'
          'GXApp/'
          'Genre'
          'GiftPurchase/'
          'HB/'
          'Header/'
          'Housing/'
          'HyperNews/'
          'INSTALL.mysql'
          'INSTALL.pgsql'
          'INSTALL_admin'
          'IPMCONTENTX/'
          'Indy_admin/'
          'Infobase/'
          'Interface/'
          'InvalidPromotion/'
          'Invitation/'
          'IssueTracker/'
          'JamPass'
          'Joomla/'
          'Jugend/'
          'KeepOut/'
          'LHC/'
          'LICENSE'
          'LICENSE/'
          'LaRaza/'
          'Lieux'
          'ListFriends/'
          'LiveUser_Admin/'
          'LoginHelp/'
          'Lotus_Domino_Admin/'
          'MAINTAINERS.txt'
          'MAINTAINERS/'
          'MEDUCATE/'
          'MR/'
          'MSADC/'
          'MSMQ/'
          'MemberProfile/'
          'Molscript/'
          'MoviesYouveSeen/'
          'MyTicketek'
          'MyTicketekArchive'
          'NSearch/'
          'Office/'
          'OrderFinal/'
          'PBSData/'
          'PBServer/'
          'POB/'
          'POBS/'
          'PSUser/'
          'Pages'
          'Parl/'
          'Paths/'
          'Pays'
          'Peopleall/'
          'PerfPassword'
          'PhotoPages/'
          'Plug-Ins/'
          'Promotion'
          'ProxyDataFeedPerformance'
          'ProxyDataFeedShowtag'
          'ProxyPriceInfo'
          'Queue/'
          'Quick/'
          'RU/'
          'Rate/'
          'RealMedia/'
          'Region'
          'Rental/'
          'Returning/'
          'SSS/'
          'SS_orders'
          'Schulung01/'
          'SearchMail/'
          'SearchOptions'
          'Seitenframes/'
          'Series'
          'Server.%EXT%'
          'Server.asp'
          'Server.html'
          'Server.php'
          'Server/'
          'ServerAdministrator/'
          'Sheldonshows'
          'Shipping/'
          'Signup/'
          'SiteDirector/'
          'SiteEdit/'
          'SiteServer'
          'Sondage'
          'Sources'
          'Stable/'
          'StaffInfo/'
          'Standardelemente/'
          'StateList'
          'States'
          'SubCategory'
          'Subjects'
          'Sujets'
          'Super-Admin/'
          'Survey'
          'SurveyAnswer'
          'SurveyAnswerOpen'
          'SurveyQuestion'
          'SurveyRespondent'
          'SysAdmin2/'
          'TWiki/'
          'Teens'
          'Telecom/'
          'Temperatur/'
          'TestInfo/'
          'Thumbs.db'
          'TimeDiff'
          'Titres'
          'ToDo/'
          'TrafficLogs/'
          'Travail/'
          'UFO'
          'UFO/'
          'USR_NAME'
          'UserFiles/'
          'UserLogin/'
          'UserPreferences'
          'VIEW1'
          'Venue'
          'VenuesNew'
          'Virus/'
          'VirusMAC/'
          'W3SVC'
          'W3SVC1'
          'W3SVC2'
          'W3SVC3'
          'W3SVC3/'
          'WEB-INF'
          'Wartung/'
          'Web2000'
          'WebBank/'
          'WebCalendar/'
          'Workflow/'
          'X_3945'
          'YourAccount/'
          'ZB/'
          'ZBmsc/'
          '_/'
          '_Layouts/'
          '_Members/'
          '_Menus/'
          '_ScriptLibrary/'
          '_Themes/'
          '___admin/'
          '__admin/'
          '__documents/'
          '_adm'
          '_adm/'
          '_adm_/'
          '_admin'
          '_admin/'
          '_admin_.php'
          '_admin_/'
          '_administrations'
          '_administrator'
          '_administrator/'
          '_administrator_/'
          '_adminpanel'
          '_adtest/'
          '_auth'
          '_backup/'
          '_baks/'
          '_borders/'
          '_cache/'
          '_client/'
          '_cms'
          '_cobrandpsp/'
          '_data/'
          '_day/'
          '_demo/'
          '_derived/'
          '_documents/'
          '_errors/'
          '_fpclass/'
          '_help/'
          '_htmlemail/'
          '_images/'
          '_includes/'
          '_install/'
          '_interface/'
          '_mem_bin/'
          '_mm/'
          '_monopoly/'
          '_mysql/'
          '_nav/'
          '_new/'
          '_notes/'
          '_notify/'
          '_old/'
          '_overlay/'
          '_pages'
          '_pages/'
          '_pcs/'
          '_photos/'
          '_pointroll/'
          '_private/'
          '_root'
          '_scripts/'
          '_server/'
          '_sess/'
          '_sessions/'
          '_staging/'
          '_testing/'
          '_tools/'
          '_update.php'
          '_utility/'
          '_virus/'
          '_visa/'
          '_voip/'
          '_vti/'
          '_vti_bin/'
          '_vti_log/'
          '_vti_pvt/'
          '_vti_script/'
          '_vti_txt/'
          '_wfspro_admin'
          'a'
          'a/'
          'a_admin'
          'aa'
          'aa/'
          'aaa'
          'aaa/'
          'aaaa'
          'aaaa/'
          'aadmin/'
          'abc'
          'abc/'
          'about'
          'about.html'
          'about.php'
          'about/'
          'aboutimages/'
          'aboutincludes/'
          'aboutstyles/'
          'abs/'
          'ac5/'
          'academic'
          'acart/'
          'acb/'
          'acc-ph/'
          'acc.txt'
          'accept/'
          'acceso.%EXT%'
          'access'
          'access.%EXT%'
          'access/'
          'access_log'
          'accessgranted'
          'accessibilitybetsie/'
          'accnt'
          'accnts'
          'account'
          'account.asp'
          'account.html'
          'account.php'
          'account/'
          'accounting'
          'accountmy/'
          'accountmypro/'
          'accountregistration/'
          'accounts'
          'accounts.%EXT%'
          'accounts/'
          'acct'
          'acct_login/'
          'achievo/'
          'acitext/'
          'action'
          'actions'
          'actions/'
          'activate/'
          'active'
          'ad'
          'ada/'
          'adap/'
          'add.php'
          'add/'
          'addnews/'
          'addressBook/'
          'addurl/'
          'addurlimage/'
          'adm'
          'adm.%EXT%'
          'adm.php'
          'adm/'
          'adm2/'
          'adm_'
          'adm_auth.%EXT%'
          'adm_manager/'
          'admcgi/'
          'admcp/'
          'admentor/'
          'admfiles/'
          'admin'
          'admin-'
          'admin-a/'
          'admin-area/'
          'admin-center/'
          'admin-elite/'
          'admin-login.%EXT%'
          'admin-serv/'
          'admin.%EXT%'
          'admin.asp'
          'admin.cfg'
          'admin.dat'
          'admin.htm'
          'admin.html'
          'admin.inc'
          'admin.php'
          'admin.pw'
          'admin/'
          'admin/account.%EXT%'
          'admin/account.asp'
          'admin/account.html'
          'admin/account.php'
          'admin/admin-login.%EXT%'
          'admin/admin.%EXT%'
          'admin/adminLogin.%EXT%'
          'admin/adminLogin.htm'
          'admin/adminLogin.html'
          'admin/admin_login.%EXT%'
          'admin/admin_login.php'
          'admin/controlpanel.%EXT%'
          'admin/controlpanel.asp'
          'admin/controlpanel.htm'
          'admin/controlpanel.html'
          'admin/controlpanel.php'
          'admin/cp.%EXT%'
          'admin/cp.asp'
          'admin/cp.html'
          'admin/cp.php'
          'admin/home.%EXT%'
          'admin/home.asp'
          'admin/home.php'
          'admin/index.asp'
          'admin/index.html'
          'admin/index.php'
          'admin/login.%EXT%'
          'admin/login.asp'
          'admin/login.htm'
          'admin/login.html'
          'admin/login.php'
          'admin/login.php/admin/login.html'
          'admin/mysql/'
          'admin/phpmyadmin/'
          'admin/pma/'
          'admin1'
          'admin1.%EXT%'
          'admin1.asp'
          'admin1.htm'
          'admin1.html'
          'admin1.php'
          'admin1/'
          'admin2'
          'admin2.%EXT%'
          'admin2.asp'
          'admin2.html'
          'admin2.php'
          'admin4/'
          'admin4_account/'
          'admin4_colon/'
          'admin525/'
          'adminLogin/'
          'adminRoot/'
          'admin_'
          'admin_.php'
          'admin_area.%EXT%'
          'admin_area/'
          'admin_area/admin.%EXT%'
          'admin_area/login.%EXT%'
          'admin_center/'
          'admin_dir'
          'admin_files/'
          'admin_id'
          'admin_includes/'
          'admin_login'
          'admin_login.%EXT%'
          'admin_logon'
          'admin_manager/'
          'admin_name'
          'admin_panel/'
          'admin_pass'
          'admin_passwd'
          'admin_password'
          'admin_pwd'
          'admin_user'
          'admin_userid'
          'admin_userinfo'
          'admin_username'
          'adminarea/'
          'admincenter/'
          'admincms'
          'admincontrol.%EXT%'
          'admincontrol.asp'
          'admincontrol.html'
          'admincontrol.php'
          'admincontrol/'
          'admincp'
          'admincp/'
          'admincp/login.%EXT%'
          'admindir'
          'adminemail'
          'adminfiles/'
          'adminid'
          'administator/'
          'administer'
          'administer/'
          'administr8.%EXT%'
          'administr8.asp'
          'administr8.html'
          'administr8.php'
          'administr8/'
          'administrable'
          'administrables'
          'administracao.php'
          'administracao/'
          'administracion.php'
          'administracion/'
          'administrador/'
          'administrare/'
          'administrarot/'
          'administrat'
          'administrate'
          'administrateur'
          'administrateur.php'
          'administrateur/'
          'administrateurs'
          'administratie/'
          'administration'
          'administration.%EXT%'
          'administration.html'
          'administration.php'
          'administration/'
          'administrations'
          'administrations_'
          'administrator'
          'administrator.%EXT%'
          'administrator.asp'
          'administrator.html'
          'administrator.php'
          'administrator/'
          'administrator/account.%EXT%'
          'administrator/account.asp'
          'administrator/account.html'
          'administrator/account.php'
          'administrator/index.asp'
          'administrator/index.html'
          'administrator/index.php'
          'administrator/login.%EXT%'
          'administrator/login.asp'
          'administrator/login.html'
          'administrator/login.php'
          'administrator_'
          'administrator_center/'
          'administrator_cp/'
          'administrator_manager/'
          'administrator_name'
          'administrator_panel/'
          'administratoraccounts/'
          'administratorcenter/'
          'administratorcp/'
          'administratorlogin.%EXT%'
          'administratorlogin/'
          'administratormanager/'
          'administratorpanel/'
          'administrators'
          'administrators.%EXT%'
          'administrators/'
          'administrer'
          'administrivia/'
          'adminitem.%EXT%'
          'adminitem/'
          'adminitems.%EXT%'
          'adminitems/'
          'adminka.php'
          'adminka/'
          'adminlogin'
          'adminlogin.%EXT%'
          'adminlogon'
          'adminmail'
          'adminmanager/'
          'adminname'
          'adminopts/'
          'adminpanel'
          'adminpanel.%EXT%'
          'adminpanel.asp'
          'adminpanel.html'
          'adminpanel.php'
          'adminpanel/'
          'adminpanel_'
          'adminphp/'
          'adminpro/'
          'adminrights'
          'admins'
          'admins.%EXT%'
          'admins.asp'
          'admins.html'
          'admins.php'
          'admins/'
          'adminsite/'
          'adminsql'
          'admintool/'
          'adminuser'
          'adminuserid'
          'adminusername'
          'admin~/'
          'admissions/'
          'admmanager/'
          'admon'
          'adodb/'
          'adodb481/'
          'ads'
          'ads/'
          'adsl'
          'advanced/'
          'advanced_search/'
          'advertise/'
          'af/'
          'afactext/'
          'affiliate.%EXT%'
          'africanimages/'
          'agent'
          'agent/'
          'agents'
          'aggregator/'
          'ai'
          'ai/'
          'aid'
          'aide'
          'aim'
          'airinfo/'
          'ajax.php'
          'ajax/'
          'al'
          'album.php'
          'album/'
          'albums/'
          'alchemist/'
          'alert/'
          'alg-geom/'
          'alias'
          'alias/'
          'aliases'
          'aliases/'
          'alignrankings/'
          'all'
          'all/'
          'allow/'
          'alls/'
          'allusers'
          'allusers/'
          'alogin.php'
          'alpha'
          'alpha/'
          'also'#9'order_log.dat'
          'alterra/'
          'am/'
          'amateur'
          'amateurs'
          'amazon/'
          'amex/'
          'amministratore.php'
          'amministratore/'
          'analog'
          'analogstats/'
          'analyse'
          'analysis/'
          'and/'
          'andtext/'
          'ani'
          'ani1'
          'anime'
          'ann'
          'ann/'
          'anne'
          'anne/'
          'announce/'
          'announcements'
          'ansichten/'
          'answer'
          'any'
          'apache'
          'apache/'
          'apectext/'
          'api'
          'api/'
          'aponline/'
          'app'
          'app/'
          'app_code/'
          'appeals/'
          'apple/'
          'applet'
          'applet/'
          'applets'
          'appliance'
          'application'
          'application/'
          'applications'
          'applications/'
          'apply'
          'appointmentstext/'
          'approved/'
          'apps'
          'apps/'
          'appz'
          'apwd'
          'arc'
          'arc/'
          'architecture/'
          'archive'
          'archive/'
          'archives'
          'archives/'
          'archivesearch/'
          'area51'
          'area51/'
          'args.bat'
          'arkiv/'
          'arreport/'
          'arrow'
          'art'
          'article.php'
          'article/'
          'article_admin'
          'articles'
          'articles.php'
          'articles/'
          'artikel'
          'arul/'
          'asian'
          'asians'
          'asiatext/'
          'askimages/'
          'asp'
          'asp/'
          'aspadmin'
          'aspnet-client/'
          'aspnet_client/'
          'asse'
          'assets'
          'assistance/'
          'astro-ph/'
          'atom-ph/'
          'attach'
          'attach/'
          'attachments'
          'attendance/'
          'aucases/'
          'auction/'
          'audinc/'
          'audio/'
          'audit'
          'audit/'
          'auid'
          'aujournals/'
          'aulegis/'
          'auother/'
          'auotherhca/'
          'auotheripaus/'
          'auotheriponline/'
          'auspecial/'
          'austlii/'
          'aut'
          'auteur'
          'auth'
          'auth.%EXT%'
          'auth.php'
          'auth/'
          'authadmin.%EXT%'
          'authenticate'
          'authenticate.%EXT%'
          'authentication'
          'authentication.%EXT%'
          'authentification'
          'authentifier'
          'author'
          'author/'
          'authorization'
          'authorlist/'
          'authuser.%EXT%'
          'auto'
          'autobackup.php'
          'autologin.%EXT%'
          'autologin/'
          'automatic'
          'autore'
          'autos/'
          'autoshipping/'
          'autotransport/'
          'av/'
          'avantgo/'
          'avhtmp/'
          'avi/'
          'aw/'
          'awStats/'
          'awald/'
          'awardsearch/'
          'awstatsoutput/'
          'b'
          'b/'
          'b2b/'
          'babelfish/'
          'back'
          'back-up'
          'backdoor'
          'backend'
          'backend/'
          'backend_users'
          'backenduser'
          'backoffice'
          'backoffice/'
          'backroom/'
          'backup'
          'backup.php'
          'backup.rar'
          'backup.sql'
          'backup.tar'
          'backup.tgz'
          'backup.txt'
          'backup.zip'
          'backup/'
          'backup0/'
          'backup2/'
          'backup3/'
          'backup4/'
          'backup5/'
          'backup6/'
          'backup7/'
          'backup8/'
          'backup9/'
          'backups'
          'backups/'
          'backupserver'
          'backupserver/'
          'bad/'
          'badbot/'
          'bak'
          'bak-up'
          'bak/'
          'bakup'
          'ban/'
          'banery/'
          'bank'
          'bank/'
          'banks'
          'banner'
          'banner/'
          'banneradmin/'
          'bannerads/'
          'banners'
          'banners/'
          'bargains/'
          'barneycamtext/'
          'barneytext/'
          'barrierefreiheit/'
          'base'
          'base/'
          'base0/'
          'base1/'
          'base2/'
          'base3/'
          'base4/'
          'base5/'
          'base6/'
          'base7/'
          'base8/'
          'base9/'
          'baseball/'
          'bases/'
          'bases0/'
          'bases1/'
          'bases2/'
          'bases3/'
          'bases4/'
          'bases5/'
          'bases6/'
          'bases7/'
          'bases8/'
          'bases9/'
          'basesearch/'
          'bash'
          'bash/'
          'bash_history'
          'bash_history/'
          'basic'
          'bass'
          'batch'
          'bay'
          'bay/'
          'bb'
          'bb-admin/'
          'bb-admin/admin.%EXT%'
          'bb-admin/admin.html'
          'bb-admin/login.%EXT%'
          'bb/'
          'bbadmin/'
          'bbb'
          'bbb/'
          'bbbb'
          'bbbb/'
          'bbcode'
          'bbcode/'
          'bbs'
          'bbs/'
          'bd'
          'bd/'
          'bdata'
          'be'
          'be/'
          'bea'
          'bean'
          'beans'
          'benifits/'
          'berube/'
          'bestform/'
          'beta'
          'beta/'
          'beweb-management/'
          'beweb/'
          'bhistory'
          'bhistory/'
          'bibliographie/'
          'bid/'
          'bigadmin/'
          'bilder/'
          'bill'
          'bill/'
          'billing'
          'billing/'
          'billpay/'
          'bin'
          'bin/'
          'binaries'
          'binary'
          'bind/'
          'bio/'
          'bios/'
          'bitmaps/'
          'bitrix/'
          'biz'
          'bizarre'
          'bizfilings/'
          'black'
          'black/'
          'blackberry/'
          'blackhistory/'
          'blackhistorytext/'
          'blank/'
          'blast/'
          'blink_temp/'
          'blinksurvey/'
          'blocks/'
          'blog'
          'blog/'
          'blog/wp-login.%EXT%'
          'blog0/'
          'blog1/'
          'blog2/'
          'blog3/'
          'blog4/'
          'blog5/'
          'blog6/'
          'blog7/'
          'blog8/'
          'blog9/'
          'blogindex/'
          'blogsearch/'
          'blow'
          'bluedome/'
          'blueprint/'
          'board'
          'board/'
          'board0/'
          'board1/'
          'board2/'
          'board3/'
          'board4/'
          'board5/'
          'board6/'
          'board7/'
          'board8/'
          'board9/'
          'boardroom/'
          'boards'
          'body'
          'book'
          'book.php'
          'book/'
          'bookmarks/'
          'bookprint/'
          'books'
          'books/'
          'boot'
          'boot/'
          'borta/'
          'boston/'
          'bot'
          'botrap/'
          'bots'
          'box'
          'boxes'
          'boy/'
          'boys/'
          'brasil/'
          'bret/'
          'brim/'
          'broadband/'
          'broken'
          'browse/'
          'browser/'
          'browsersync/'
          'bsd'
          'bsd/'
          'bts/'
          'budge/'
          'budget/'
          'budgettext/'
          'bug'
          'bug/'
          'bug0/'
          'bug1/'
          'bug2/'
          'bug3/'
          'bug4/'
          'bug5/'
          'bug6/'
          'bug7/'
          'bug8/'
          'bug9/'
          'bugs'
          'bugs/'
          'build'
          'builder'
          'bulk'
          'bus/'
          'business'
          'businessFaqs/'
          'business_temp/'
          'businessadvantage/'
          'businesscharts/'
          'businessimages/'
          'businessincludes/'
          'businesspricingtool/'
          'businessstyles/'
          'buttons'
          'buy.php'
          'buy/'
          'bz/'
          'c'
          'c/'
          'c0de/'
          'c4sure/'
          'c99.php'
          'cache'
          'cache/'
          'cachemgr'
          'cad'
          'cadmins/'
          'calendar/'
          'calendarcontrol/'
          'calendarfeeds/'
          'calendarical/'
          'calendartext/'
          'call/'
          'camadmin'
          'camera/'
          'camera0/'
          'camera1/'
          'camera2/'
          'camera3/'
          'camera4/'
          'camera5/'
          'camera6/'
          'camera7/'
          'camera8/'
          'camera9/'
          'cameraoffer/'
          'camerapromo/'
          'camserver'
          'camserver/'
          'can'
          'canada/'
          'cantonS/'
          'cantonadmin/'
          'cantonimg/'
          'cantoninc/'
          'captcha'
          'captchas/'
          'capturetool/'
          'car'
          'car/'
          'card'
          'card/'
          'cardinal'
          'cards'
          'cards/'
          'career/'
          'careers/'
          'carmover/'
          'carpet'
          'carsdirect/'
          'carshipper/'
          'carstransport/'
          'cart'
          'cart.php'
          'cart/'
          'cartconfig/'
          'cartransport/'
          'cas'
          'cas/'
          'case/'
          'cash/'
          'cat'
          'catalog'
          'catalog/'
          'catalog_list/'
          'catalogs'
          'catalogs/'
          'catalogues/'
          'catch'
          'catchup/'
          'categories/'
          'category/'
          'categorygeneral/'
          'cb/'
          'cb5/'
          'cbi-bin/'
          'cbo/'
          'cbphp/'
          'cc'
          'cc.txt'
          'cc/'
          'cc_expires'
          'cc_number'
          'cc_owner'
          'cc_type'
          'ccbill/'
          'ccc'
          'ccc/'
          'cccc'
          'cccc/'
          'cchimages/'
          'ccms/'
          'ccms/index.php'
          'ccms/login.php'
          'ccp14admin/'
          'ccs'
          'ccupdate/'
          'cd'
          'cdrom'
          'cdrom/'
          'cdrom0/'
          'cdrom1/'
          'cdrom2/'
          'cdrom3/'
          'cdrom4/'
          'cdrom5/'
          'cdrom6/'
          'cdrom7/'
          'cdrom8/'
          'cdrom9/'
          'ceatext/'
          'ceilidh2/'
          'ceiva/'
          'center/'
          'center0/'
          'center1/'
          'center2/'
          'center3/'
          'center4/'
          'center5/'
          'center6/'
          'center7/'
          'center8/'
          'center9/'
          'centre/'
          'centuryslides/'
          'ceqtext/'
          'cerberus/'
          'cert'
          'certenroll'
          'certif'
          'certificate'
          'certificates'
          'certified'
          'certify'
          'certs'
          'certs/'
          'certsrv/'
          'cfdocs'
          'cfg'
          'cfg.php'
          'cfg.txt'
          'cfkarchive/'
          'cfm/'
          'cfusion/'
          'cgi'
          'cgi-'
          'cgi-/'
          'cgi-914/'
          'cgi-915/'
          'cgi-auth/'
          'cgi-bin'
          'cgi-bin/'
          'cgi-bin/login%EXT%'
          'cgi-bin2/'
          'cgi-csc/'
          'cgi-exe/'
          'cgi-home/'
          'cgi-lib/'
          'cgi-local/'
          'cgi-perl/'
          'cgi-scripts/'
          'cgi-secure/'
          'cgi-shl/'
          'cgi-shop/'
          'cgi-src/'
          'cgi-sys/'
          'cgi-temp/'
          'cgi-win'
          'cgi-win/'
          'cgi.cgi'
          'cgi.cgi/'
          'cgi/'
          'cgi_bin/'
          'cgibin'
          'cgibin/'
          'cgis/'
          'ch/'
          'chaines/'
          'chan'
          'change'
          'changelog'
          'changelog.txt'
          'changepw'
          'changeset/'
          'channel'
          'chao/'
          'chart'
          'chart/'
          'chat'
          'chat/'
          'chat_config'
          'chat_messages'
          'chat_users'
          'check.%EXT%'
          'check/'
          'checkadmin.%EXT%'
          'checklogin.%EXT%'
          'checkout/'
          'checkpoint'
          'checkpoint/'
          'checks.txt'
          'checkuser.%EXT%'
          'chef/'
          'cheftext/'
          'chefvideo/'
          'chem-ph/'
          'chene/'
          'cheneimg/'
          'cheneinc/'
          'chordsimages/'
          'chordstemp/'
          'cid'
          'cinfo/'
          'cir/'
          'citrix/'
          'cits/'
          'ck.log'
          'cl.txt'
          'cl2feeds/'
          'cl2ical/'
          'class'
          'class/'
          'classes'
          'classes/'
          'classic'
          'classified'
          'classified/'
          'classifieds'
          'clickheat/'
          'clicknbuild/'
          'client'
          'client/'
          'client_templates/'
          'clientname'
          'clientpassword'
          'clients'
          'clients/'
          'clientusername'
          'clock/'
          'closeup/'
          'clubconfig'
          'cluster'
          'cm'
          'cmd'
          'cmp-lg/'
          'cms'
          'cms/'
          'cms0/'
          'cms1/'
          'cms2/'
          'cms3/'
          'cms4/'
          'cms5/'
          'cms6/'
          'cms7/'
          'cms8/'
          'cms9/'
          'cms_admin'
          'cms_admins'
          'cms_user'
          'cms_users'
          'cmsadmin'
          'cmsadmin.%EXT%'
          'cmsadmin/'
          'cmsincludes/'
          'cnet/'
          'cnstats/'
          'cnt/'
          'co_intra/'
          'coalition/'
          'cobalt'
          'cobalt/'
          'cobrand/'
          'code'
          'code/'
          'codes'
          'codes/'
          'codesearch/'
          'codestariff/'
          'coding'
          'coffee'
          'coke'
          'college/'
          'com/'
          'comics/'
          'command'
          'command/'
          'commandes'
          'comment/'
          'commentary/'
          'commentreply/'
          'comments/'
          'commerce'
          'commercial'
          'commercial/'
          'common'
          'communities/'
          'community/'
          'comp/'
          'comp0/'
          'comp1/'
          'comp2/'
          'comp3/'
          'comp4/'
          'comp5/'
          'comp6/'
          'comp7/'
          'comp8/'
          'comp9/'
          'company'
          'company/'
          'compendium/'
          'competition/'
          'complet'
          'complete/'
          'component'
          'component/'
          'components/'
          'compose'
          'composer'
          'compressed'
          'compte'
          'comptes'
          'computer_info/'
          'comunicator'
          'con'
          'concessionnaire'
          'concessionnaires'
          'cond-mat/'
          'conf'
          'conf/'
          'conf0/'
          'conf1/'
          'conf2/'
          'conf3/'
          'conf4/'
          'conf5/'
          'conf6/'
          'conf7/'
          'conf8/'
          'conf9/'
          'confidential'
          'confidential/'
          'config'
          'config-old/'
          'config.bak'
          'config.inc'
          'config.inc.bak'
          'config.inc.old'
          'config.inc.php'
          'config.inc.php.txt'
          'config.inc.txt'
          'config.ini'
          'config.ini.bak'
          'config.ini.old'
          'config.ini.txt'
          'config.old'
          'config.php'
          'config.php.bak'
          'config.php.inc'
          'config.php.old'
          'config.php.txt'
          'config.txt'
          'config/'
          'config0/'
          'config1/'
          'config2/'
          'config3/'
          'config4/'
          'config5/'
          'config6/'
          'config7/'
          'config8/'
          'config9/'
          'configs'
          'configuration'
          'configuration/'
          'configure'
          'configure/'
          'configure0/'
          'configure1/'
          'configure2/'
          'configure3/'
          'configure4/'
          'configure5/'
          'configure6/'
          'configure7/'
          'configure8/'
          'configure9/'
          'connect'
          'connect.php'
          'connect/'
          'connected/'
          'connections'
          'connexion'
          'conquer/'
          'console'
          'console/'
          'constant'
          'constants'
          'consumer/'
          'cont/'
          'contact'
          'contact/'
          'contacts'
          'contacts.html'
          'contacts/'
          'content'
          'content.php'
          'content/'
          'contentnewsadmn/'
          'contents'
          'contents/'
          'contenu'
          'contrast/'
          'contrib/'
          'control'
          'control.%EXT%'
          'control/'
          'controle'
          'controller'
          'controlpanel'
          'controlpanel.%EXT%'
          'controlpanel.asp'
          'controlpanel.html'
          'controlpanel.php'
          'controlpanel/'
          'controls'
          'controls/'
          'convenience/'
          'cook/'
          'cookie/'
          'cookies/'
          'cooks/'
          'cool.php'
          'coolsites/'
          'coop/'
          'coopdirectory/'
          'coord/'
          'corba'
          'core'
          'coremetrics/'
          'coremsgsimages/'
          'corp'
          'corp/'
          'corporate'
          'corporate/'
          'count'
          'count/'
          'counter'
          'counter/'
          'counters/'
          'country/'
          'countrywide/'
          'counts/'
          'cp'
          'cp.%EXT%'
          'cp.asp'
          'cp.html'
          'cp.php'
          'cp/'
          'cpanel'
          'cpanel.php'
          'cpanel/'
          'cpanel_file/'
          'cpg_config'
          'cpropubunivercd/'
          'cps/'
          'crack'
          'cracker'
          'cracker/'
          'crackers'
          'crackers/'
          'cracks'
          'create'
          'creation'
          'credit'
          'creditcards'
          'crm'
          'crm/'
          'cron'
          'cron.php'
          'cron/'
          'cross/'
          'crownjewels/'
          'crs'
          'csimg/'
          'css'
          'css/'
          'csv'
          'ctc'
          'ctflohmarkt/'
          'ctforen/'
          'ctforum/'
          'cubaimages/'
          'custom/'
          'customer'
          'customer/'
          'customer_login/'
          'customers'
          'customers/'
          'customers_basket'
          'customers_email_address'
          'customers_password'
          'customize'
          'customize/'
          'customs/'
          'cv'
          'cv/'
          'cvsweb/'
          'cvvnumber'
          'cybercash/'
          'cybertext/'
          'd'
          'd/'
          'daemon'
          'daemon/'
          'daily/'
          'darcsweb/'
          'dash/'
          'dat'
          'dat/'
          'data'
          'data/'
          'data0/'
          'data1/'
          'data2/'
          'data3/'
          'data4/'
          'data5/'
          'data6/'
          'data7/'
          'data8/'
          'data9/'
          'dataDir/'
          'database'
          'database.inc'
          'database/'
          'database0/'
          'database1/'
          'database2/'
          'database3/'
          'database4/'
          'database5/'
          'database6/'
          'database7/'
          'database8/'
          'database9/'
          'databaseapplication/'
          'databases'
          'databases/'
          'databases0/'
          'databases1/'
          'databases2/'
          'databases3/'
          'databases4/'
          'databases5/'
          'databases6/'
          'databases7/'
          'databases8/'
          'databases9/'
          'datadump/'
          'dataimages/'
          'dataprog/'
          'datas/'
          'datatemplates/'
          'dav'
          'day/'
          'daytext/'
          'db'
          'db-admin'
          'db.inc'
          'db/'
          'db00/'
          'db01'
          'db01/'
          'db02'
          'db02/'
          'db03'
          'db03/'
          'db04/'
          'db05/'
          'db06/'
          'db07/'
          'db08/'
          'db09/'
          'db1'
          'db1/'
          'db2'
          'db2/'
          'db2s/'
          'db3'
          'db3/'
          'db4/'
          'db5/'
          'db6/'
          'db7/'
          'db8/'
          'db9/'
          'db_database_name'
          'db_hostname'
          'db_password'
          'db_username'
          'dba'
          'dbadmin'
          'dbadmin/'
          'dbadmins'
          'dbase'
          'dbauth/'
          'dbcompitems/'
          'dbm'
          'dbms'
          'dbpages/'
          'dcontent/'
          'dd'
          'dd/'
          'ddd'
          'ddd/'
          'dddd'
          'dddd/'
          'ddddd/'
          'de/'
          'dealer'
          'dealers'
          'dean/'
          'debug'
          'deepsight/'
          'default'
          'default.html'
          'default/'
          'defaultpage/'
          'defaultpages/'
          'defaults/'
          'delete'
          'delete.php'
          'deletion'
          'demo'
          'demo/'
          'demo0/'
          'demo1/'
          'demo2/'
          'demo3/'
          'demo4/'
          'demo5/'
          'demo6/'
          'demo7/'
          'demo8/'
          'demo9/'
          'demobilder/'
          'demodiskett/'
          'demos'
          'demos/'
          'demosite/'
          'deny'
          'deny/'
          'deploy'
          'deployment'
          'depository/'
          'descs/'
          'design'
          'design/'
          'designs'
          'designs/'
          'desktop'
          'desktop/'
          'details'
          'detectiveimages/'
          'dev'
          'dev/'
          'dev0/'
          'dev1'
          'dev1/'
          'dev2/'
          'dev3/'
          'dev4/'
          'dev5/'
          'dev6/'
          'dev60cgi'
          'dev7/'
          'dev8/'
          'dev9/'
          'devel'
          'develop'
          'develop/'
          'develop0/'
          'develop1/'
          'develop2/'
          'develop3/'
          'develop4/'
          'develop5/'
          'develop6/'
          'develop7/'
          'develop8/'
          'develop9/'
          'developement'
          'developers'
          'development'
          'development/'
          'device'
          'devices'
          'devil'
          'devil/'
          'devlnull'
          'devlnull/'
          'devs'
          'dg-ga/'
          'dhtml'
          'diag'
          'dial'
          'dialogs/'
          'diary'
          'dict/'
          'dieband/'
          'dienst/'
          'dig'
          'dig/'
          'digcam/'
          'digcamoffer/'
          'digital/'
          'digitaloup/'
          'digitalwork/'
          'digitalworks/'
          'diplomacy/'
          'dir'
          'dir-login/'
          'dir.php'
          'dir/'
          'directadmin/'
          'directories/'
          'directory'
          'directory/'
          'dirs'
          'dirs/'
          'discover/'
          'discovery'
          'discus/'
          'disk'
          'disk/'
          'diskett/'
          'disks/'
          'disney/'
          'dispatch'
          'dispatcher'
          'dispatches/'
          'display/'
          'distr/'
          'distrib/'
          'distributions/'
          'div/'
          'diversity/'
          'dl/'
          'dldc/'
          'dll'
          'dll/'
          'dlls/'
          'dm/'
          'dmr'
          'dmr/'
          'dmr1'
          'dms'
          'dns'
          'do/'
          'do0/'
          'do1/'
          'do2/'
          'do3/'
          'do4/'
          'do5/'
          'do6/'
          'do7/'
          'do8/'
          'do9/'
          'doc'
          'doc-html/'
          'doc/'
          'doc0/'
          'doc1/'
          'doc2/'
          'doc3/'
          'doc4/'
          'doc5/'
          'doc6/'
          'doc7/'
          'doc8/'
          'doc9/'
          'docs'
          'docs/'
          'docs0/'
          'docs00/'
          'docs01/'
          'docs02/'
          'docs03/'
          'docs04/'
          'docs05/'
          'docs06/'
          'docs07/'
          'docs08/'
          'docs09/'
          'docs1/'
          'docs2/'
          'docs3/'
          'docs4/'
          'docs41'
          'docs5/'
          'docs51'
          'docs6/'
          'docs7/'
          'docs8/'
          'docs9/'
          'document'
          'document/'
          'document0/'
          'document1/'
          'document2/'
          'document3/'
          'document4/'
          'document5/'
          'document6/'
          'document7/'
          'document8/'
          'document9/'
          'documents'
          'documents/'
          'documents0/'
          'documents1/'
          'documents2/'
          'documents3/'
          'documents4/'
          'documents5/'
          'documents6/'
          'documents7/'
          'documents8/'
          'documents9/'
          'dog/'
          'doom'
          'doom/'
          'door'
          'double'
          'dougelliman/'
          'down'
          'down/'
          'down_load/'
          'down_loads/'
          'download'
          'download.php'
          'download/'
          'downloads'
          'downloads/'
          'downloadsBrowse/'
          'downloadsinfo/'
          'downloadz'
          'dpanel/'
          'dpctext/'
          'draft'
          'dragon'
          'dratfs'
          'dreamteam/'
          'dreamweaver'
          'dreamweaver/'
          'dremin/'
          'driver'
          'drivers'
          'dropsafe/'
          'dsl/'
          'dtproperties'
          'dtree/'
          'du/'
          'dump'
          'dump.sql'
          'dump.tar'
          'dump.tgz'
          'dump.zip'
          'dumpenv'
          'dvi/'
          'e'
          'e-bay'
          'e-bay/'
          'e-mail'
          'e-print/'
          'e/'
          'e107_handlers/'
          'e107_user'
          'e5/'
          'e_bay/'
          'e_mail'
          'earn/'
          'earthlinkmall/'
          'earthlinkvsaol/'
          'eastendersupdates/'
          'easter/'
          'eastertext/'
          'easy'
          'easyweb/'
          'ebay'
          'ebay/'
          'ebill/'
          'ebriefs'
          'ebuseconmacroecon/'
          'echannel'
          'echas/'
          'echat/'
          'ecom/'
          'ecommerce'
          'ecommercemm/'
          'ecomtext/'
          'economytext/'
          'ecostructure/'
          'edit'
          'edit.php'
          'editor'
          'editor.php'
          'editor/'
          'editors/'
          'education/'
          'efriend/'
          'egovtext/'
          'eintrag/'
          'ekeith/'
          'element'
          'elements'
          'elements/'
          'elink/'
          'elink030600/'
          'elinkoffers/'
          'elinks/'
          'elite.php'
          'elite/'
          'elnadvertise/'
          'elnbook/'
          'elnk_uunet/'
          'elnkmall/'
          'email'
          'email/'
          'email_addresses/'
          'emailaddress'
          'emailalert/'
          'emailauth/'
          'emailmock/'
          'emailmock2/'
          'emailmock3/'
          'emailprint/'
          'emailpromo/'
          'emails'
          'emails/'
          'emailspecial/'
          'emailtext/'
          'ememories/'
          'emer'
          'emi/'
          'emni'
          'emniplote'
          'employees'
          'empty/'
          'emri'
          'en'
          'en/'
          'en_US'
          'enbusiness/'
          'enchannels/'
          'enchmeetings/'
          'enclosemover/'
          'encommon/'
          'encrm/'
          'endtmeetings/'
          'ene/'
          'enemail/'
          'energy/'
          'energytext/'
          'enescapeaways/'
          'enesmeetings/'
          'eng'
          'eng/'
          'engine'
          'engine/'
          'engineering/'
          'english'
          'engroups/'
          'engs/'
          'enh/'
          'enhiltonuniversity/'
          'enhimeetings/'
          'enhotels/'
          'enincludes/'
          'eninfo/'
          'enlogin/'
          'enmandataire/'
          'enmy/'
          'enmyprofile/'
          'enplansoft/'
          'enpromotions/'
          'enpsvocadmin/'
          'enregistrs'
          'enreservations/'
          'enross/'
          'ensearch/'
          'ensignup/'
          'ensim'
          'ensim/'
          'enspot/'
          'ensweepstakes/'
          'enterchatroom/'
          'enterprise'
          'enterprise/'
          'entm/'
          'entravel/'
          'entreprise'
          'entry'
          'env'
          'environ'
          'environment'
          'eoltools/'
          'epa/'
          'epages/'
          'epoch/'
          'eqinchen/'
          'erp'
          'erp/'
          'error'
          'error/'
          'error_log'
          'errordocs/'
          'errorpages/'
          'errors'
          'errors/'
          'es'
          'esales'
          'escapades/'
          'esd/'
          'eservices/'
          'esp'
          'espanoltext/'
          'essa/'
          'established'
          'esupport'
          'esurance/'
          'etc'
          'etc/'
          'etc/security/'
          'etcsecurity/'
          'eth0'
          'eth0/'
          'eth1/'
          'eth2/'
          'eth3/'
          'eth4/'
          'eth5/'
          'eue/'
          'events'
          'events/'
          'evite/'
          'evs/'
          'ews/'
          'example'
          'example/'
          'examples'
          'examples/'
          'excellencetext/'
          'exchange'
          'exe'
          'exe/'
          'exec'
          'exec/'
          'execsummit/'
          'executable'
          'executables'
          'exmo/'
          'experts/'
          'exploits/'
          'explore/'
          'explorer'
          'expo/'
          'export'
          'export/'
          'extension.inc'
          'extern/'
          'external'
          'external/'
          'extra'
          'extras/'
          'extrasforen/'
          'ezbackup/'
          'ezerror/'
          'ezicart/'
          'ezsqliteadmin/'
          'f'
          'f/'
          'face/'
          'fadm/'
          'fail'
          'failed'
          'fair/'
          'fantasy/'
          'faq.html'
          'faq.php'
          'faq.txt'
          'faq/'
          'faqs/'
          'far/'
          'farmpub/'
          'fashion/'
          'fastbin/'
          'fastforward/'
          'fastlane/'
          'fastlanemac/'
          'favicon.ico'
          'fb_only/'
          'fbcitext/'
          'fcgi-/'
          'fcgi-bin'
          'fcgi-bin/'
          'featured/'
          'features/'
          'feber02/'
          'feed/'
          'feedback'
          'feedback/'
          'feeds/'
          'fekonja/'
          'fellowstext/'
          'field'
          'file'
          'file.html'
          'file.php'
          'file/'
          'file_id.diz'
          'fileadmin'
          'fileadmin.%EXT%'
          'fileadmin.asp'
          'fileadmin.html'
          'fileadmin.php'
          'fileadmin/'
          'filehunter'
          'filehunter/'
          'files'
          'files.html'
          'files/'
          'filez'
          'filez.html'
          'film'
          'filme/'
          'films'
          'films/'
          'filter'
          'final'
          'finance/'
          'financial/'
          'find/'
          'firewall'
          'first'
          'first.php'
          'first/'
          'fissure'
          'fitnesstext/'
          'fix/'
          'fjalekalimi'
          'fjalekalimin'
          'fkat/'
          'fla'
          'fla/'
          'flags/'
          'flash'
          'flash/'
          'flashed/'
          'flashtesttext/'
          'flashtext/'
          'flow_chart/'
          'flu/'
          'flut/'
          'flutext/'
          'flv/'
          'flvtext/'
          'fm/'
          'foiaincludes/'
          'folder'
          'follow/'
          'fonts/'
          'foo'
          'food'
          'food/'
          'footer/'
          'for_inf/'
          'ford/'
          'fordtext/'
          'foren/'
          'forenadmin/'
          'forenattach_mod/'
          'forendb/'
          'forenfiles/'
          'forenprofile/'
          'forget'
          'forgot'
          'forgotten'
          'form'
          'form/'
          'form_back/'
          'format'
          'format/'
          'formhandler'
          'formmail/'
          'forms/'
          'forms_directory/'
          'formsend'
          'formslogin/'
          'formular/'
          'formularz/'
          'formupdate'
          'forthehome/'
          'fortune'
          'forum'
          'forum/'
          'forum2/'
          'forum_phpbb/'
          'forums'
          'forums/'
          'forwrite/'
          'foto/'
          'fotos/'
          'foundry'
          'fpdb/'
          'fr/'
          'frame'
          'framework'
          'framework/'
          'free'
          'free.php'
          'freebusy/'
          'freecgi/'
          'freepic'
          'freepics'
          'freescan/'
          'freescanone/'
          'freeware/'
          'front'
          'frontline/'
          'frontpage'
          'frontpage/'
          'froogle/'
          'fs/'
          'fsbrprint/'
          'fsbrtext/'
          'fscripts/'
          'ftp'
          'ftp.txt'
          'ftp/'
          'ftps/'
          'fuck'
          'fucks'
          'full'
          'full/'
          'fullsizecover/'
          'fun'
          'fun/'
          'funct-an/'
          'function'
          'function.inc'
          'function.php'
          'functional.php'
          'functions'
          'fungi/'
          'funktionen/'
          'furniture/'
          'fusion_user_groups'
          'fusion_users'
          'g'
          'g/'
          'gal'
          'galerie1/'
          'gallerie/'
          'galleries'
          'gallery'
          'gallery/'
          'gallery2/'
          'galls'
          'game'
          'games'
          'games/'
          'gamez'
          'garden/'
          'gardens/'
          'gardenstext/'
          'gardentext/'
          'gate'
          'gateway'
          'gateway/'
          'gateways/'
          'gb/'
          'gbrochu/'
          'gcc/'
          'gems/'
          'general/'
          'generic'
          'generic/'
          'geo/'
          'geoTrack/'
          'gest'
          'gestionnaire'
          'get'
          'get.php'
          'get/'
          'get_linked/'
          'getbook/'
          'getlinked/'
          'getrich/'
          'getty'
          'getty/'
          'gh/'
          'ghost/'
          'ghosts/'
          'ghoststext/'
          'gid'
          'gif/'
          'gifs'
          'gifs/'
          'giftgiant/'
          'girl'
          'girl.html'
          'girl/'
          'girls'
          'girls.html'
          'girls/'
          'girlz'
          'girlz.html'
          'give.php'
          'glconnection/'
          'global'
          'global.php'
          'global/'
          'globala-en/'
          'globalall/'
          'globalam/'
          'globalar/'
          'globalcl/'
          'globalco/'
          'globalcr/'
          'globaleg/'
          'globalemea/'
          'globalid/'
          'globalma/'
          'globalmx/'
          'globalmy/'
          'globalnav'
          'globalpe/'
          'globalph/'
          'globalpr/'
          'globals'
          'globalsg/'
          'globaltext/'
          'globalth/'
          'globalus/'
          'globalve/'
          'globes_admin/'
          'go/'
          'go2/'
          'godiva/'
          'gold/'
          'gone'
          'goodbye/'
          'google'
          'google/'
          'googleearth'
          'googleearth/'
          'googlesite/'
          'gopherdata/'
          'goto/'
          'goto0/'
          'goto1/'
          'goto2/'
          'goto3/'
          'goto4/'
          'goto5/'
          'govdoc/'
          'govdoc0/'
          'govdoc1/'
          'govdoc2/'
          'govdoc3/'
          'govdoc4/'
          'govdoc5/'
          'gp'
          'gp/'
          'gpapp'
          'gpcart/'
          'gpflex/'
          'gr-qc/'
          'gra/'
          'grafik/'
          'grafiken/'
          'granted'
          'graph'
          'graphic'
          'graphics'
          'graphics/'
          'graphs'
          'grddl/'
          'greet/'
          'greeting/'
          'greetings/'
          'greybox/'
          'grill/'
          'ground/'
          'group'
          'group/'
          'group0/'
          'group00/'
          'group1/'
          'group_name'
          'groupcp.php'
          'groupe'
          'groupes'
          'groups'
          'groups/'
          'groupspersonalized/'
          'groupwise'
          'groupwise/'
          'gsmp/'
          'guardian/'
          'guardian_Backup/'
          'gue/'
          'guest'
          'guest-demos/'
          'guest/'
          'guestbook'
          'guestbook/'
          'guests'
          'guests/'
          'gui/'
          'gutenberg/'
          'gwharburg/'
          'gwt/'
          'gytext/'
          'h'
          'h/'
          'h4ck/'
          'hachage'
          'hack'
          'hack/'
          'hacked'
          'hacked/'
          'hacker'
          'hacker/'
          'hacks'
          'hacks/'
          'haddan_files/'
          'haha/'
          'hakusyo/'
          'halloween/'
          'handler'
          'hangaroo/'
          'hanlder'
          'hao/'
          'happening'
          'hardcore'
          'hash/'
          'hashsalt'
          'head'
          'header'
          'header.php'
          'headers'
          'headlines/'
          'helena/'
          'hello'
          'helloworld'
          'help'
          'help/'
          'helpdesk/'
          'helper/'
          'hep/'
          'hept/'
          'hex/'
          'hfolkedmmbhf/'
          'hfolkedmmbseritest/'
          'hh/'
          'hiMOULTHROP/'
          'hidden'
          'hide'
          'hide/'
          'hiltonpride/'
          'hiltonpride_dir/'
          'history'
          'history/'
          'historya/'
          'historytext/'
          'hits'
          'hl/'
          'hlt/'
          'hm/'
          'hni/'
          'holiday/'
          'home'
          'home-networking/'
          'home.html'
          'home/'
          'homepage'
          'homes'
          'homework'
          'honor/'
          'horde/'
          'host'
          'host/'
          'hosting/'
          'hosts'
          'hosts/'
          'hotels/'
          'house/'
          'house2/'
          'houseads/'
          'hpwebjetadmin/'
          'hr'
          'hr/'
          'hrmrpl/'
          'hsctext/'
          'htaccess'
          'htaccess/'
          'htbin/'
          'htdata'
          'htdig-dev/'
          'htdig/'
          'htdoc'
          'htdocs'
          'htm'
          'htm/'
          'html'
          'html/'
          'html_errors/'
          'htmls'
          'htpasswd'
          'htpasswrd'
          'htsdata/'
          'http_errors/'
          'hunt/'
          'hwdev/'
          'hwdq/'
          'hws/'
          'i'
          'i-mode/'
          'i/'
          'ia/'
          'ibank/'
          'ibf_admin_sessions'
          'ibf_conf_settings'
          'ibf_members'
          'ibf_members_converge'
          'ibf_sessions'
          'ibill/'
          'ibm'
          'ice/'
          'ico/'
          'icons'
          'icons/'
          'iconssmall/'
          'icq-info.txt'
          'icq.log'
          'icq.txt'
          'icq/'
          'icq_number'
          'icw/'
          'id'
          'id_group'
          'id_member'
          'idbc'
          'ids'
          'ie/'
          'ie5/'
          'iedbbare_edb/'
          'iedbwwwlunsj/'
          'ieie40/'
          'ientertain/'
          'ig/'
          'ignatiusj'
          'iis'
          'iisadmin/'
          'iisadmpwd/'
          'iishelp/'
          'iissamples/'
          'im/'
          'ima/'
          'image'
          'image.php'
          'image/'
          'images'
          'images/'
          'images_dir'
          'imagestext/'
          'imagestore/'
          'imagez'
          'ime'
          'img'
          'img/'
          'imgres/'
          'immoscan/'
          'imode/'
          'imodesearch/'
          'imp/'
          'import'
          'import.txt'
          'import/'
          'in/'
          'inbox'
          'inbox/'
          'inc'
          'inc/'
          'incimages/'
          'include'
          'include/'
          'include_content/'
          'includefiles/'
          'includes'
          'includes/'
          'incoming'
          'incoming/'
          'incoming_files/'
          'incs'
          'incs/'
          'index'
          'index.aspx'
          'index.dhtml'
          'index.htm'
          'index.html'
          'index.php'
          'index.py'
          'index.shtml'
          'index/'
          'index1.php'
          'index2'
          'index2.html'
          'index3.html'
          'index_.php'
          'index_adm'
          'index_admin'
          'indexes'
          'indexes/'
          'inf.php'
          'info'
          'info.php'
          'info/'
          'infocen/'
          'infocentre/'
          'infocus/'
          'infocustomerror/'
          'information'
          'information/'
          'informer/'
          'inforobots/'
          'infoslides/'
          'infosmart404/'
          'infosource/'
          'infraot/'
          'ingres'
          'ingress'
          'ini'
          'ini/'
          'init'
          'init/'
          'input'
          'input/'
          'ins/'
          'inside'
          'inside/'
          'inside_earthlink/'
          'instadmin/'
          'install'
          'install.mysql.txt'
          'install.pgsql.txt'
          'install.php'
          'install.sql'
          'install.txt'
          'install/'
          'install_.php'
          'installation'
          'installation.php'
          'installation/'
          'insurance/'
          'int/'
          'interactive'
          'interdoc/'
          'internal'
          'internal/'
          'international/'
          'internet'
          'internet/'
          'intl_kb/'
          'intlitapreports/'
          'intlkb/'
          'intra'
          'intra/'
          'intranet'
          'intranet/'
          'intro'
          'inventory'
          'invitation'
          'invite'
          'iol/'
          'ip/'
          'ip_address'
          'ipb_sessions'
          'ipo/'
          'ipp'
          'ipro/'
          'ips'
          'ips_kernel/'
          'irc-macadmin/'
          'irc/'
          'isadmin.%EXT%'
          'isallow/'
          'isapi/'
          'ishide/'
          'itBO/'
          'itestadisticas/'
          'itmonitoreo/'
          'itmr/'
          'itpm/'
          'ius/'
          'iweb/'
          'iwebCTW/'
          'j'
          'j/'
          'jackets/'
          'java'
          'java-sys'
          'java/'
          'javascript'
          'javascript/'
          'javascripts/'
          'javaserver'
          'javaserver/'
          'javsascript/'
          'jay/'
          'jd/'
          'jdbc'
          'jdbc/'
          'jira/'
          'job'
          'jobs/'
          'john'
          'john/'
          'johnmthistle/'
          'join'
          'joke.php'
          'joke/'
          'joomla'
          'joomla_users'
          'jos_users'
          'journal'
          'journal/'
          'journals/'
          'jp/'
          'jpg/'
          'jpgs/'
          'jref/'
          'jrun'
          'js'
          'js/'
          'jsapi/'
          'jscript/'
          'jscripts/'
          'jscss/'
          'jserv/'
          'jsky/'
          'jsp'
          'jsp/'
          'jsps'
          'jsr'
          'jukebox/'
          'just/'
          'k'
          'k/'
          'kaffee/'
          'keep'
          'kentesting/'
          'kept'
          'kernel'
          'kernel/'
          'key'
          'key/'
          'keye/'
          'keys/'
          'keyword/'
          'kibo/'
          'kickstart/'
          'kids/'
          'kidsApps/'
          'kidsabctext/'
          'kidzone/'
          'kielce/'
          'kiosque/'
          'kmail/'
          'knews_lostpass'
          'kodi'
          'komtel/'
          'kongjian/'
          'kontakt/'
          'korea/'
          'korisnici'
          'korisnik'
          'kpanel/'
          'kpro_adminlogs'
          'kpro_user'
          'krakow/'
          'kurs/'
          'kw/'
          'kyam/'
          'l'
          'l/'
          'l0gin.php'
          'lab'
          'labadmin/'
          'labprivate/'
          'labpurgatory/'
          'labs'
          'labs/'
          'ladder/'
          'ladies/'
          'laiskai/'
          'lan'
          'lan/'
          'lancelot'
          'lang/'
          'language/'
          'largescale/'
          'last_ip'
          'last_login'
          'lastname'
          'lastname/'
          'launch'
          'launchpage'
          'layout/'
          'lbsbackup/'
          'ldap'
          'learning/'
          'leet'
          'leet/'
          'left'
          'les'
          'lesb'
          'lesb.html'
          'lesbian'
          'lesbian.html'
          'lesbians'
          'lesbians.html'
          'lesbo'
          'letmein.%EXT%'
          'letmein/'
          'letters'
          'letters/'
          'level'
          'lez'
          'lez.html'
          'lib'
          'lib/'
          'libraries'
          'libraries/'
          'library'
          'library/'
          'libs'
          'license.txt'
          'liebhart/'
          'liens'
          'link'
          'link/'
          'linklogo/'
          'links'
          'links.html'
          'links/'
          'links1.html'
          'links2.html'
          'linkz'
          'linux'
          'linux/'
          'list'
          'list/'
          'lists'
          'lists/'
          'lite/'
          'live'
          'live/'
          'livetechtext/'
          'livetext/'
          'livre'
          'llogaria'
          'load'
          'load.php'
          'loader'
          'local/'
          'local_/'
          'local_url/'
          'lock'
          'lockout'
          'log'
          'log-in.%EXT%'
          'log-in/'
          'log.php'
          'log/'
          'log_in.%EXT%'
          'log_in/'
          'log_order/'
          'log_orders/'
          'loganalysis/'
          'logfile'
          'logfiles'
          'logfiles/'
          'logger'
          'logging'
          'login'
          'login%EXT%'
          'login-redirect/'
          'login-us/'
          'login.%EXT%'
          'login.asp'
          'login.htm'
          'login.html'
          'login.php'
          'login/'
          'login1%EXT%'
          'login1/'
          'login_admin'
          'login_admin%EXT%'
          'login_admin/'
          'login_admins'
          'login_db/'
          'login_name'
          'login_out%EXT%'
          'login_out/'
          'login_pass'
          'login_passwd'
          'login_password'
          'login_pw'
          'login_pwd'
          'login_user'
          'login_user%EXT%'
          'login_username'
          'login_users'
          'loginerror/'
          'loginflat/'
          'logini'
          'loginkey'
          'loginok/'
          'loginout'
          'logins'
          'logins/'
          'loginsave/'
          'loginsuper%EXT%'
          'loginsuper/'
          'logo'
          'logo.jpg'
          'logo_sysadmin/'
          'logohu'
          'logon'
          'logon/'
          'logout'
          'logout%EXT%'
          'logout/'
          'logs'
          'logs-a/'
          'logs.php'
          'logs/'
          'lol/'
          'look/'
          'lookup'
          'lookup/'
          'lost%2Bfound'
          'lost_pass'
          'lost_passwords'
          'lostpass'
          'lostpasswords'
          'lotus'
          'lotus/'
          'love'
          'love/'
          'loyalty/'
          'lozinka'
          'lrvk/'
          'ls'
          'lugindex/'
          'lxr/'
          'm'
          'm/'
          'm3communications/'
          'm_admin'
          'mac/'
          'macadmin/'
          'magic'
          'maia/'
          'mail'
          'mail-admin.nsf'
          'mail/'
          'mailbox'
          'maildata/'
          'maillist'
          'maillist/'
          'mailman/'
          'mailroot/'
          'mails'
          'mails/'
          'mailto_addresses/'
          'main'
          'main/'
          'maint'
          'maintenance/'
          'make'
          'make/'
          'makefile'
          'makefile/'
          'mall/'
          'mall_log_files/'
          'mallpage/'
          'mallpromo/'
          'mambo'
          'mambo/'
          'mambo_session'
          'mambo_users'
          'mambots/'
          'man'
          'manage'
          'manage.%EXT%'
          'manage/'
          'management'
          'management.%EXT%'
          'management/'
          'manager'
          'manager.%EXT%'
          'manager/'
          'manga'
          'mangas'
          'manninen/'
          'mans/'
          'manual'
          'manual/'
          'manuallogin/'
          'manuals/'
          'map'
          'map.php'
          'map/'
          'maple/'
          'maprogress/'
          'maps/'
          'marisa/'
          'mark'
          'mark/'
          'market'
          'marketing'
          'mary'
          'mary/'
          'master'
          'master/'
          'mastergate/'
          'math-ph/'
          'math/'
          'matrix'
          'matrix/'
          'mb_users'
          'mbd/'
          'mbo'
          'mbox/'
          'mchat/'
          'md5hash'
          'mdb'
          'mdp/'
          'me'
          'meadmin/'
          'mebius/'
          'mecss/'
          'media/'
          'mediafiles'
          'mediafiles/'
          'mediastore/'
          'mediatext/'
          'mem_login'
          'member'
          'member.%EXT%'
          'member/'
          'member_htpasswd'
          'member_htpasswd/'
          'member_id'
          'member_info/'
          'member_login_key'
          'member_name'
          'memberadmin.%EXT%'
          'memberadmin/'
          'memberbenunsubscribe/'
          'memberid'
          'memberlist'
          'membername'
          'members'
          'members-only'
          'members.%EXT%'
          'members.php'
          'members/'
          'membership/'
          'membre'
          'membres'
          'membres_dev/'
          'memlogin/'
          'memo/'
          'memorial/'
          'memory'
          'mempassword'
          'menu'
          'menu/'
          'meppimages/'
          'mepuretxt/'
          'merc/'
          'merchant/'
          'merger/'
          'mescripts/'
          'mesearch/'
          'message'
          'messageboards/'
          'messages'
          'messages/'
          'messaging'
          'messy'
          'meta'
          'meta_login/'
          'metabase'
          'metadata/'
          'metriweb/'
          'mgr'
          'microsoft/'
          'mid/'
          'migrationinfo/'
          'mih/'
          'mihw/'
          'mindware/'
          'mine'
          'minibbtable_users'
          'minimum'
          'mirctutorial/'
          'mirror'
          'mirror/'
          'mirrors'
          'mirrors/'
          'misc'
          'misc/'
          'misc5daytest/'
          'missing/'
          'mitglieder'
          'mitigationss/'
          'miura_h/'
          'mkstats'
          'ml/'
          'mm/'
          'mng'
          'mnp_utility/'
          'mo/'
          'moa/'
          'mobile/'
          'mock/'
          'mockups/'
          'mod'
          'mod/'
          'modcp/'
          'model'
          'modelsearch/'
          'modelsearch/login.%EXT%'
          'modelsearch/login.asp'
          'modelsearch/login.php'
          'modem'
          'moderation.php'
          'moderator.%EXT%'
          'moderator.asp'
          'moderator.html'
          'moderator.php'
          'moderator/'
          'moderator/admin.%EXT%'
          'moderator/admin.asp'
          'moderator/admin.html'
          'moderator/admin.php'
          'moderator/login.%EXT%'
          'moderator/login.asp'
          'moderator/login.html'
          'moderator/login.php'
          'modern/'
          'moderntail/'
          'moderntails/'
          'mods'
          'mods/'
          'module'
          'module/'
          'modules'
          'modules.php'
          'modules/'
          'modules/admin/'
          'molmovdb/'
          'mon'
          'mon_mot_de_passe'
          'money/'
          'monitor'
          'monitors'
          'monmotdepasse'
          'mot'#9'de'#9'passe'
          'motd/'
          'motorola_old/'
          'mount'
          'movabletype/'
          'movie'
          'movies'
          'movies/'
          'moy/'
          'mp3'
          'mp3/'
          'mp3s'
          'mpcgi/'
          'mqseries'
          'mrtg'
          'mrtg/'
          'ms'
          'ms-sql'
          'msacct/'
          'mschat/'
          'msdw/'
          'msearch/'
          'msg/'
          'msglobal/'
          'mshelp/'
          'msinger/'
          'msn'
          'msn/'
          'msnetstatus/'
          'mspgpcflowers/'
          'msql'
          'msservice/'
          'mssql'
          'mssql/'
          'mssup/'
          'mssupport/'
          'mssupportfaqs/'
          'mssupportlive/'
          'mtr/'
          'multimedia/'
          'museum/'
          'music'
          'music/'
          'musica/'
          'musicfiles'
          'musicfiles/'
          'musics/'
          'my'
          'my-sql'
          'my/'
          'my_email'
          'my_name'
          'my_password'
          'my_username'
          'myaccount/'
          'myadmin'
          'myadmin/'
          'mybb_users'
          'mydownloads'
          'mydownloads/'
          'myfamily/'
          'myfiles'
          'myfiles/'
          'myie/'
          'myname'
          'mypan/'
          'mypassword'
          'myphpadmin/'
          'mysql'
          'mysql-admin/'
          'mysql.user'
          'mysql/'
          'mysql_admin/'
          'mysqladmin/'
          'mysqldumper/'
          'mysqlinfo.php'
          'mysqlinfo.shtml'
          'mystical/'
          'mytext/'
          'myuploads'
          'myuploads/'
          'myusername'
          'n'
          'n/'
          'n031337f1l3/'
          'n0F1l33ndn3v3rw45'
          'n0F1l33ndn3v3rw45/'
          'name'
          'names'
          'napster/'
          'nav/'
          'navSiteAdmin/'
          'navigate/'
          'navigation'
          'navigation/'
          'navigator/'
          'navtree/'
          'navytext/'
          'nc'
          'ncbar/'
          'ncube/'
          'ne'
          'nectext/'
          'neo'
          'neo/'
          'net'
          'net/'
          'netartmatch/'
          'netartmongrel/'
          'netbilling/'
          'netcat/'
          'nethelp/'
          'netscape'
          'netserver/'
          'netstat'
          'nettools/'
          'netware'
          'netware/'
          'network'
          'network/'
          'networking/'
          'netz/'
          'new'
          'new.php'
          'new/'
          'newads/'
          'newclients/'
          'newpsp/'
          'news'
          'news.php'
          'news/'
          'news_lostpass'
          'newsadmin/'
          'newsletter'
          'newssummary/'
          'newtech/'
          'newtext/'
          'newticket/'
          'newtorents'
          'newtorents/'
          'next'
          'nf2004text/'
          'nfipInsurance/'
          'nfuseclients/'
          'nfuseicons/'
          'nice/'
          'nick'
          'nickelnights/'
          'nieuws/'
          'night'
          'night/'
          'nights/'
          'nl'
          'nobody'
          'node/'
          'nodeadd/'
          'nom'
          'nombre'
          'noms'
          'noonecansee_bia/'
          'normakamali/'
          'normakamalipromo/'
          'notebook/'
          'notebooksearch/'
          'notes'
          'notices/'
          'notify/'
          'nouveau'
          'novell'
          'novell/'
          'nowe/'
          'nowires/'
          'noworries/'
          'nowy/'
          'nsc/'
          'nsciraq/'
          'nsctext/'
          'nt'
          'nt/'
          'nucl/'
          'nuke_authors'
          'nuke_bbconfig'
          'nuke_config'
          'nuke_popsettings'
          'nuke_users'
          'nul'
          'null'
          'null.html'
          'null/'
          'number'
          'nummer'
          'nurfuer/'
          'nwa/'
          'nwshp/'
          'ny/'
          'nyc/'
          'nytimes-partners/'
          'nytimes/'
          'nz/'
          'nzcases/'
          'o'
          'o/'
          'oa/'
          'oai1/'
          'oai2/'
          'ob/'
          'obb_profiles'
          'object'
          'objectif'
          'objects'
          'objects/'
          'obsolete/'
          'odbc'
          'odbc/'
          'odds/'
          'oepcguil5de/'
          'oepcra32g6ws/'
          'oetaki/'
          'of'
          'off'
          'office'
          'official/'
          'ofs'
          'ogl'
          'oh'
          'ojp/'
          'ok/'
          'old'
          'old/'
          'older'
          'oldie'
          'oldies'
          'oldintranet/'
          'oldsite/'
          'oldtars/'
          'ombud/'
          'omniture/'
          'omo/'
          'on'
          'onairsamdonaldson/'
          'one'
          'one/'
          'onecore/'
          'onine_shop/'
          'online'
          'online.php'
          'online/'
          'onlycern/'
          'oop/OoO'
          'oopOoO/'
          'oordir'
          'open'
          'open/'
          'openapp'
          'opendb/'
          'opendir/'
          'opendoor/'
          'openfile'
          'openvpnadmin/'
          'openwebmail/'
          'opera/'
          'operator'
          'oplata/'
          'oppna_samlingar/'
          'opt/'
          'option/'
          'options/'
          'opts/'
          'oracle'
          'oracle/'
          'oradata'
          'order'
          'order.log'
          'order.old/'
          'order.txt'
          'order/'
          'order_log'
          'order_log_v12.dat'
          'order_temp/'
          'ordernow/'
          'orders'
          'orders.log'
          'orders.txt'
          'orders/'
          'orders_log'
          'orders_temp/'
          'ordre'
          'org/'
          'orig/'
          'original/'
          'originals/'
          'orkutalbums/'
          'oscommerce'
          'oscommerce/'
          'osg/'
          'oss/'
          'oswald/'
          'other/'
          'otherartscentre/'
          'others.php'
          'others/'
          'otto/'
          'ottox/'
          'outbound/'
          'outgoing'
          'outils/'
          'outlook'
          'outlook/'
          'output'
          'outside'
          'outside/'
          'ov/'
          'ows-bin/'
          'p'
          'p/'
          'p2p'
          'p2p/'
          'p_assword'
          'p_word'
          'pad'
          'page'
          'page/'
          'pagead/'
          'pager'
          'pager/'
          'pages/'
          'pages/admin/'
          'pages/admin/admin-login.%EXT%'
          'pagescollege/'
          'painel/'
          'paintingtheweatherzoom/'
          'pairs/'
          'palace/'
          'palm/'
          'pam'
          'pamhackett/'
          'panel'
          'panel-administracion/'
          'panel-administracion/login.%EXT%'
          'panel.%EXT%'
          'panel.php'
          'panel/'
          'paper'
          'paper_passwd/'
          'papers'
          'parallel/'
          'paramtres'
          'parol'
          'partenaire'
          'partenaires'
          'partenaires/'
          'partial/'
          'partner/'
          'partners/'
          'pass'
          'pass.txt'
          'pass/'
          'pass1word'
          'pass_hash'
          'pass_w'
          'pass_word'
          'passe'
          'passer'
          'passes'
          'passport/'
          'passw'
          'passwd'
          'passwd.adjunct'
          'passwd.txt'
          'passwd/'
          'passwor'
          'password'
          'password.txt'
          'password/'
          'password_manager/'
          'passwords'
          'passwords/'
          'passwordsalt'
          'passwordz'
          'passwort'
          'passwrd'
          'patagon/'
          'patents/'
          'path'
          'path/'
          'patt/'
          'pays/'
          'pcflowers/'
          'pcoSpotlight/'
          'pda/'
          'pdasearch/'
          'pdf'
          'pdf/'
          'pdfs_docs/'
          'pdg_cart/'
          'peanut/'
          'pear/'
          'penya/'
          'people/'
          'peoplechat/'
          'peoplesoft'
          'peoplesoft/'
          'perdorimi'
          'perdorues'
          'perdoruesi'
          'perdoruesit'
          'perl'
          'perl/'
          'perl5'
          'perl5/'
          'pers'
          'pers/'
          'perso'
          'perso.html'
          'personal'
          'personal/'
          'personal_key'
          'personals'
          'personnal'
          'pes.old/'
          'petest/'
          'pfiab/'
          'pgadmin/'
          'pgsql'
          'phone'
          'phone/'
          'phones/'
          'phorum'
          'phorum/'
          'phorum_session'
          'phorum_user'
          'phorum_users'
          'photo'
          'photo/'
          'photoPhoto/'
          'photobanner/'
          'photobbs/'
          'photocounter/'
          'photodaniela/'
          'photoemail/'
          'photoform/'
          'photogalery/'
          'photohana/'
          'photohome/'
          'photoikone/'
          'photoimages/'
          'photoindex_old/'
          'photoline/'
          'photolinkovi/'
          'photolinks/'
          'photomain/'
          'photonikola/'
          'photonina/'
          'photophotos/'
          'photorobots/'
          'photorollover/'
          'photos'
          'photos/'
          'photosadrzaj/'
          'photoscroll/'
          'photoshop'
          'photoshop/'
          'photoslika/'
          'photoslike/'
          'phototemp/'
          'phototerragen/'
          'photovlinto/'
          'photowebmaster/'
          'photowebslike/'
          'php'
          'php.ini'
          'php.php'
          'php/'
          'phpBB/'
          'phpMyA/'
          'phpMyAdmi/'
          'phpMyAdmin-2.10.0/'
          'phpMyAdmin-2.2.3/'
          'phpMyAdmin-2.2.6/'
          'phpMyAdmin-2.3.0/'
          'phpMyAdmin-2.3.1/'
          'phpMyAdmin-2.3.2/'
          'phpMyAdmin-2.3.3/'
          'phpMyAdmin-2.3.4/'
          'phpMyAdmin-2.3.5/'
          'phpMyAdmin-2.3.6/'
          'phpMyAdmin-2.3.7/'
          'phpMyAdmin-2.3.8/'
          'phpMyAdmin-2.3.9/'
          'phpMyAdmin-2.4.0/'
          'phpMyAdmin-2.4.1/'
          'phpMyAdmin-2.4.2/'
          'phpMyAdmin-2.4.3/'
          'phpMyAdmin-2.4.4/'
          'phpMyAdmin-2.4.5/'
          'phpMyAdmin-2.4.6/'
          'phpMyAdmin-2.4.7/'
          'phpMyAdmin-2.4.8/'
          'phpMyAdmin-2.4.9/'
          'phpMyAdmin-2.5.0/'
          'phpMyAdmin-2.5.1/'
          'phpMyAdmin-2.5.2/'
          'phpMyAdmin-2.5.3/'
          'phpMyAdmin-2.5.4/'
          'phpMyAdmin-2.5.5/'
          'phpMyAdmin-2.5.6/'
          'phpMyAdmin-2.5.7/'
          'phpMyAdmin-2.5.8/'
          'phpMyAdmin-2.5.9/'
          'phpMyAdmin-2.6.0-pl1/'
          'phpMyAdmin-2.6.0/'
          'phpMyAdmin-2.6.1/'
          'phpMyAdmin-2.6.2-rc1/'
          'phpMyAdmin-2.6.2/'
          'phpMyAdmin-2.6.3-pl1/'
          'phpMyAdmin-2.6.3-rc1/'
          'phpMyAdmin-2.6.3/'
          'phpMyAdmin-2.6.4/'
          'phpMyAdmin-2.6.5/'
          'phpMyAdmin-2.6.6/'
          'phpMyAdmin-2.6.7/'
          'phpMyAdmin-2.6.8/'
          'phpMyAdmin-2.6.9/'
          'phpMyAdmin-2.7.0/'
          'phpMyAdmin-2.7.1/'
          'phpMyAdmin-2.7.2/'
          'phpMyAdmin-2.7.3/'
          'phpMyAdmin-2.7.4/'
          'phpMyAdmin-2.7.5/'
          'phpMyAdmin-2.7.6/'
          'phpMyAdmin-2.7.7/'
          'phpMyAdmin-2.7.8/'
          'phpMyAdmin-2.7.9/'
          'phpMyAdmin-2.8.1/'
          'phpMyAdmin-2.8.2/'
          'phpMyAdmin-2.8.3/'
          'phpMyAdmin-2.8.4/'
          'phpMyAdmin-2.8.5/'
          'phpMyAdmin-2.8.6/'
          'phpMyAdmin-2.8.7/'
          'phpMyAdmin-2.8.8/'
          'phpMyAdmin-2.8.9/'
          'phpMyAdmin-2.9.1/'
          'phpMyAdmin-2.9.2/'
          'phpMyAdmin-2/'
          'phpMyAdmin-3/'
          'phpMyAdmin-4/'
          'phpMyAdmin1/'
          'phpMyAdmin_old/'
          'phpMyAds/'
          'phpSQLiteAdmin/'
          'phpSecure/'
          'phpWahl/'
          'php_docs/'
          'php_uploads/'
          'phpadmin/'
          'phpads_clients'
          'phpads_config'
          'phpbb'
          'phpbb_users'
          'phpboard'
          'phpboard/'
          'phpchat/'
          'phpfunctions/'
          'phpinfo.php'
          'phpinfo.php.txt'
          'phpldapadmin/'
          'phplib/'
          'phpm/'
          'phpmy/'
          'phpmyad-sys/'
          'phpmyad/'
          'phpmyadm/'
          'phpmyadmin'
          'phpmyadmin-old/'
          'phpmyadmin/'
          'phpmyadmin/phpinfo.php'
          'phpmyadmin/scripts/setup.php'
          'phpmyadmin2/'
          'phpnewsspider/'
          'phpnuke'
          'phpnuke/'
          'phppfriendly/'
          'phppgadmin/'
          'phpproject/'
          'phpsess/'
          'phpsessid/'
          'phpshell.php'
          'phpsysinfo/'
          'phpwebalbum/'
          'physics/'
          'pic'
          'pic.html'
          'pic/'
          'picpost'
          'picposts'
          'pics'
          'pics/'
          'picture'
          'picture.html'
          'picture/'
          'picture_library/'
          'pictures'
          'pictures.html'
          'pictures/'
          'picturez'
          'picz'
          'pierre'#9'liste'
          'ping'
          'pipermail/'
          'pix'
          'pix/'
          'pl'
          'plasma/'
          'platz_login/'
          'plesk-stat'
          'plesk-stat/'
          'pls'
          'plug'
          'plug/'
          'plugins'
          'plugins/'
          'plx'
          'pm/'
          'pma/'
          'pma_table_info'
          'pnTemp/'
          'pntables/'
          'pobeda/'
          'podcast/'
          'podcasts/'
          'pol'
          'policy'
          'policy/'
          'poll'
          'poll/'
          'poll_user'
          'polly'
          'polly/'
          'pop'
          'pop/'
          'popup/'
          'popupdate/'
          'porn'
          'porn/'
          'port/'
          'portal'
          'portal/'
          'portalapp/'
          'portalhome/'
          'portlet'
          'portlets'
          'portugal/'
          'possix'
          'possix/'
          'post'
          'post/'
          'postcard/'
          'postgres'
          'postgres/'
          'postnuke'
          'postnuke/'
          'postnuke/html'
          'power'
          'power_user/'
          'powered/'
          'powerplus/'
          'powerseller'
          'powerseller/'
          'ppc/'
          'pqa/'
          'prazdnik/'
          'pre'
          'pre/'
          'preferences/'
          'prelive'
          'prelive/'
          'president/'
          'presidenttext/'
          'press'
          'presse/'
          'pressehistory/'
          'pressofficeplots/'
          'preview'
          'preview/'
          'previews'
          'previewz'
          'prezident/'
          'principal'
          'print'
          'print/'
          'printable/'
          'printenv'
          'printer/'
          'printers/'
          'printtext/'
          'priv'
          'priv.html'
          'priv/'
          'priv8/'
          'privacy'
          'privacy.html'
          'privacy.php'
          'privacy/'
          'privat/'
          'private'
          'private.html'
          'private.php'
          'private/'
          'privs'
          'pro/'
          'probando'
          'problem/'
          'procedures/'
          'process'
          'processform'
          'processlogin.%EXT%'
          'prod'
          'product'
          'product/'
          'producten/'
          'production'
          'production/'
          'products'
          'products/'
          'products_/'
          'productsfinder/'
          'productslearnaction/'
          'professor'
          'profile'
          'profile/'
          'profiles/'
          'prog'
          'program'
          'programs'
          'programs/'
          'project'
          'project-admins/'
          'project/'
          'projects/'
          'promo/'
          'promos/'
          'proof'
          'properties'
          'property/'
          'protect'
          'protected'
          'prova'
          'prova1'
          'prova2'
          'provas'
          'proxy'
          'proxy/'
          'prueba'
          'prueba00'
          'prueba01'
          'prueba1'
          'prueba2'
          'pruebas'
          'prv'
          'ps'
          'ps/'
          'psd'
          'psd/'
          'psds'
          'psds/'
          'psfigs/'
          'psp/'
          'psp_ads/'
          'psw'
          'ptmenu/'
          'pub'
          'pub/'
          'public'
          'public/'
          'public_html/'
          'publish'
          'publisher'
          'pulse/'
          'punbb_users'
          'punetoret'
          'punonjes'
          'puppy'
          'purchase'
          'purchases'
          'purchases/'
          'pureadmin/'
          'purgatory/'
          'put'
          'pw'
          'pw/'
          'pwd'
          'pwds'
          'pwiki/'
          'pword'
          'pwrd'
          'python'
          'q'
          'q-alg/'
          'q/'
          'qa/'
          'qqq/'
          'quake'
          'quake/'
          'quant/'
          'query'
          'query.html'
          'queue'
          'quikstore.cfg'
          'quikstore.cgi'
          'quitaol/'
          'quote'
          'qwe/'
          'r'
          'r/'
          'r57.php'
          'radio/'
          'radmind-1/'
          'radmind/'
          'rail/'
          'raj/'
          'ramon'
          'random'
          'random/'
          'rank'
          'raw/'
          'rcLogin/'
          'rcs'
          'rd/'
          'rdf/'
          'rdp/'
          're/'
          'read/'
          'read_me.txt'
          'reader/'
          'readme'
          'readme.html'
          'readme.txt'
          'reagan/'
          'reality/'
          'realm/'
          'recherche'
          'recycle_bin/'
          'redditchmatters/'
          'redesign/'
          'redir'
          'redir/'
          'redirect'
          'redirect/'
          'redirection/'
          'refer/'
          'reference'
          'references'
          'referencesforums/'
          'referrals/'
          'reflect/'
          'refs/'
          'reg'
          'reg.php'
          'reg_user'
          'reg_users'
          'reginternal'
          'regional'
          'regions/'
          'register'
          'register/'
          'registered'
          'registration/'
          'registry.php'
          'reguser'
          'regusers'
          'reklama/'
          'related/'
          'relax/'
          'relcontent/'
          'release'
          'relogin.%EXT%'
          'relogin.htm'
          'relogin.html'
          'relpage/'
          'remind'
          'reminder'
          'remote'
          'remote/'
          'removed'
          'rep/'
          'replace/'
          'reply/'
          'report'
          'report/'
          'reports'
          'reports/'
          'repositories/'
          'request/'
          'requisite'
          'res/'
          'research'
          'research/'
          'reseller'
          'resnet/'
          'resource'
          'resource/'
          'resources'
          'resources/'
          'responder'
          'restricted'
          'restricted/'
          'result/'
          'results/'
          'retail'
          'retscreen/'
          'reunions/'
          'reuters/'
          'review'
          'reviews'
          'reviews/'
          'reviewssearch/'
          'ribi/'
          'ricochet/'
          'right'
          'rita/'
          'roam/'
          'robert'
          'robert/'
          'robot'
          'robot.txt'
          'robotics'
          'robots.txt'
          'rocketman/'
          'roll/'
          'roller/'
          'rolltext/'
          'roman/'
          'room/'
          'root'
          'root.php'
          'root/'
          'rotaryconference/'
          'route'
          'router'
          'rpc'
          'rpc/'
          'rrr.php'
          'rsjlibraryrciadic/'
          'rss'
          'rss/'
          'rssfull/'
          'rsstext/'
          'rules'
          'run'
          'rupay/'
          's'
          's/'
          'sa/'
          'safebrowsing/'
          'safesolutions/'
          'saiyou/'
          'sales'
          'salt'
          'saltususa/'
          'sample'
          'sample/'
          'samples'
          'samples/'
          'sams/'
          'sandpit/'
          'sara/'
          'save'
          'save/'
          'saved'
          'saves/'
          'sbot/'
          'scalix/'
          'scan/'
          'scanners/'
          'scgi-bin/'
          'schach/'
          'schema'
          'scholar/'
          'scm/'
          'scr'
          'scratc'
          'screens/'
          'script'
          'scripts'
          'scripts/'
          'sdk'
          'search'
          'search.php'
          'search/'
          'searchVolltext/'
          'searchhistory/'
          'searchtools/'
          'sec/'
          'secret'
          'secret/'
          'secretanswer'
          'secretquestion'
          'secrets'
          'secrets/'
          'section'
          'sections'
          'sectionspopoff/'
          'secure'
          'secure/'
          'secured'
          'security'
          'security.php'
          'security/'
          'seek/'
          'sefl/'
          'sel'
          'select'
          'selinux/'
          'sell'
          'seller'
          'seller/'
          'send'
          'send/'
          'sendarticle/'
          'sendmail'
          'sendmail/'
          'sensepost'
          'sensor'
          'sent'
          'seo/'
          'ser/'
          'serial'
          'serials'
          'serialz'
          'server'
          'server-status/'
          'server1'
          'server1/'
          'server2'
          'server2/'
          'server_admin_small/'
          'server_stats'
          'serverbackup'
          'serverbackup/'
          'servers'
          'servers/'
          'service'
          'service/'
          'services'
          'services/'
          'servlet'
          'servlet/'
          'servlets'
          'servlets/'
          'sess/'
          'session'
          'session/'
          'session_member_id'
          'session_member_login_key'
          'sessions'
          'sessions/'
          'sesskey'
          'set'
          'set.html'
          'set/'
          'setprefs/'
          'sets/'
          'setting'
          'setting/'
          'settings'
          'settings/'
          'setup'
          'setup.php'
          'setup.sql'
          'setup/'
          'sex'
          'sex/'
          'sexy'
          'sgarchives/'
          'sgml/'
          'share'
          'share/'
          'sharebuilder/'
          'shared'
          'shared/'
          'sharepoint'
          'sharepoint/'
          'shares'
          'shares/'
          'shareware/'
          'shell'
          'shell.php'
          'shell.txt'
          'shell/'
          'shemales'
          'shengdanjie/'
          'shipping.php'
          'shipsdoctor/'
          'shit'
          'shop'
          'shop.cards'
          'shop.orders'
          'shop.php'
          'shop/'
          'shopbot/'
          'shopper'
          'shopper.conf'
          'shopping/'
          'show'
          'showcode'
          'showfiles.php'
          'showlogin/'
          'shtml'
          'shutdown/'
          'sid'
          'siebel'
          'siebel/'
          'sifra'
          'sigint/'
          'sign'
          'sign-in.%EXT%'
          'sign-in/'
          'sign_in.%EXT%'
          'sign_in/'
          'signature'
          'signin'
          'signin.%EXT%'
          'signin/'
          'silva/'
          'silver/'
          'simple'
          'simple.php'
          'simpleLogin/'
          'simplicity/'
          'single'
          'sisterhazel/'
          'sit/'
          'site'
          'site/'
          'site_admin/'
          'site_login'
          'site_logins'
          'site_manager/'
          'siteadmin.%EXT%'
          'siteadmin/'
          'sitedown/'
          'sitelogin'
          'sitelogins'
          'sitemanager/'
          'sitemap'
          'sitemap.html'
          'sitemap.xml'
          'sitemap/'
          'sites'
          'sites/'
          'sitesearch/'
          'skins/'
          'slave/'
          'slidesexamples/'
          'small'
          'small/'
          'smallnuke_members'
          'smallworld/'
          'smblogin/'
          'smc/'
          'smf_members'
          'smile/'
          'smoelen/'
          'smoelenboek/'
          'sms/'
          'smsdemo/'
          'smtp'
          'smtp/'
          'sna/'
          'snac/'
          'snoop'
          'snort'
          'snort/'
          'soap'
          'soapdocs'
          'social/'
          'socks/'
          'soft.php'
          'soft/'
          'software'
          'software/'
          'software_club/'
          'soho/'
          'solaris'
          'solaris/'
          'solitaire/'
          'solution'
          'solutions'
          'solutions/'
          'solv/'
          'somebody'
          'sondage/'
          'songs/'
          'sorry/'
          'sort'
          'sos/'
          'sotu/'
          'sou/'
          'sound/'
          'source'
          'source.php'
          'source/'
          'sources/'
          'space/'
          'spaceIntern/'
          'spaceconfig/'
          'spacer'
          'spacer/'
          'spain'
          'spam'
          'spam/'
          'spamcop'
          'spamcop/'
          'spaminator/'
          'spanish'
          'sparc/'
          'spaw/'
          'spec/'
          'special/'
          'specials/'
          'spider'
          'spider/'
          'spidered'
          'spidered/'
          'spiders'
          'spiders/'
          'spipecrire/'
          'splace/'
          'splash/'
          'sport/'
          'sportstext/'
          'sprint/'
          'sprint1000/'
          'sprintpcs/'
          'sprintverify/'
          'sql'
          'sql-admin/'
          'sql.sql'
          'sql.tar'
          'sql.tgz'
          'sql.txt'
          'sql.zip'
          'sql/'
          'sqladmin'
          'sqladmin/'
          'squid'
          'squid/'
          'squirellmail'
          'squirellmail/'
          'squirrelmail/'
          'src'
          'src/'
          'srch/'
          'srchad'
          'srchadm/'
          'srv'
          'srv/'
          'ss.php'
          'ss_vms_admin_sm/'
          'ss_vms_admin_sm/admin/account.html/ad'
          'mi'
          'n/index.php'
          'ss_vms_admin_sm/admin1.php'
          'ssh.txt'
          'sshadmin/'
          'ssi'
          'ssi/'
          'ssl'
          'ssl/'
          'st/'
          'stadtteilplaene/'
          'staff'
          'staff/'
          'staffweb/'
          'stage'
          'stage/'
          'stage1'
          'stage1/'
          'stager'
          'stager/'
          'standard/'
          'staradmin/'
          'start'
          'start/'
          'start_page/'
          'startpage'
          'startpage/'
          'stat'
          'stat/'
          'statestext/'
          'static/'
          'statistic'
          'statistics'
          'statistics/'
          'statistiques'
          'stats'
          'stats/'
          'statsreportmagic/'
          'status'
          'status/'
          'stemcell/'
          'sthb/'
          'stoage/'
          'stock'
          'stock/'
          'stocker'
          'stockphoto'
          'stockphoto/'
          'stocks'
          'stocks/'
          'stone'#9'list'
          'stonemanor/'
          'stop'
          'store'
          'store/'
          'store1'
          'stored'
          'stored/'
          'storemgr.pw'
          'stores/'
          'story'
          'strategiutv/'
          'strategy/'
          'string'
          'stud/'
          'student'
          'studier/'
          'stuff'
          'stuff.html'
          'stuff/'
          'stuffed/'
          'style'
          'style.css'
          'style/'
          'styles/'
          'stylesheet'
          'stylesheets'
          'styletest/'
          'stylist/'
          'sub-login/'
          'submit'
          'submit/'
          'submitfiles/'
          'submitter'
          'subscriptions/'
          'such_alt/'
          'suche/'
          'suchen/'
          'sujets/'
          'summerschool/'
          'sun'
          'super'
          'super%EXT%'
          'super1%EXT%'
          'super1/'
          'super_index%EXT%'
          'super_login%EXT%'
          'superfly/'
          'superman%EXT%'
          'superman/'
          'supermanager%EXT%'
          'superuser'
          'superuser%EXT%'
          'superuser.%EXT%'
          'superuser/'
          'supervise/'
          'supervise/Login%EXT%'
          'supervisor/'
          'support'
          'support/'
          'support_login/'
          'supported'
          'supportee/'
          'supr/'
          'suradio/'
          'survey/'
          'susan'
          'susan/'
          'svc'
          'svc/'
          'svn'
          'svn/'
          'svr'
          'sw'
          'sweeps/'
          'swf'
          'swf/'
          'switch/'
          'switchover/'
          'switchoverfiles/'
          'swr/'
          'syncro/'
          'syndicationhtml/'
          'sys'
          'sys-admin/'
          'sys/'
          'sysadm.%EXT%'
          'sysadm/'
          'sysadmin'
          'sysadmin.%EXT%'
          'sysadmin.asp'
          'sysadmin.html'
          'sysadmin.php'
          'sysadmin/'
          'sysadmins'
          'sysadmins/'
          'sysconstraints'
          'sysinfo/'
          'syssegments'
          'system'
          'system-administration/'
          'system/'
          'system32/'
          'system_administration/'
          'system_manager/'
          'systema/'
          'systemadm/'
          'systemadmin/'
          'systemadministration/'
          'systeminfo/'
          'systems/'
          'systest/'
          'systime'
          'sysuser'
          'sysusers'
          'szab-test/'
          'szabdatabase/'
          'szabprotect/'
          't'
          't/'
          't900/'
          'tab'
          'table'
          'table_prefix'
          'tables'
          'tabs/'
          'tacdir/'
          'tag'
          'tag/'
          'tagebuch/'
          'takeovers/'
          'talkingpoints/'
          'talks/'
          'talkway/'
          'tape'
          'tar'
          'target'
          'tavle/'
          'taxonomy/'
          'tb/'
          'td/'
          'team/'
          'teamearthlink/'
          'teamelink/'
          'tech'
          'technologygoto/'
          'techtrans/'
          'teen'
          'teenz'
          'teer/'
          'telecharger'
          'telecom4u/'
          'temp'
          'temp/'
          'temp_order/'
          'temp_orders/'
          'temp_pass'
          'temp_password'
          'template'
          'template/'
          'templates'
          'templates/'
          'templates_c/'
          'temporal'
          'temporary/'
          'temppass'
          'temppasword'
          'temps'
          'terminal'
          'terminal/'
          'termine/'
          'terms.php'
          'terms/'
          'terrori/'
          'test'
          'test-cgi/'
          'test.htm'
          'test.html'
          'test.php'
          'test/'
          'test0.php'
          'test00'
          'test01'
          'test1'
          'test1.php'
          'test12/'
          'test2'
          'test2.php'
          'test3.php'
          'test4.php'
          'test5.php'
          'test6.php'
          'test7.php'
          'test8.php'
          'test9.php'
          'test_/'
          'test_banner/'
          'test_website/'
          'testads/'
          'testclive/'
          'testdp/'
          'tester'
          'testfm/'
          'testharness/'
          'testing'
          'testing.php'
          'testing/'
          'testisc/'
          'testkj/'
          'testlogs/'
          'testmartin/'
          'testmw/'
          'testpam/'
          'testphpinfo.php'
          'testps/'
          'testrobot/'
          'testrobots/'
          'testrobotsdisal/'
          'testrobotsnoindex/'
          'tests'
          'tests/'
          'testtad/'
          'testtext/'
          'text'
          'text/'
          'texte'
          'texts'
          'textview/'
          'tgz/'
          'thearena/'
          'theatre/'
          'thecell/'
          'theearthlink/'
          'themall/'
          'theme/'
          'themes/'
          'thestreet/'
          'thisweek/'
          'thumb'
          'thumbs'
          'thumbz'
          'ticker/'
          'ticket'
          'tir/'
          'tirctw/'
          'tirstyle/'
          'titleGraphic/'
          'titles/'
          'tmp'
          'tmp/'
          'tmp1/'
          'tmpfile/'
          'today'
          'together/'
          'tom'
          'tom/'
          'tool'
          'toolbar'
          'toolbar/'
          'toolbars/'
          'tools'
          'tools.php'
          'tools/'
          'toolscoverup/'
          'toolswhois/'
          'toolz/'
          'top'
          'top/'
          'topics'
          'torch'
          'torch/'
          'torrent'
          'torrent/'
          'torrents'
          'torrents/'
          'tour'
          'tovers/'
          'tpforen/'
          'tpl/'
          'tpv'
          'trace'
          'trace/'
          'track/'
          'trackback/'
          'tracker'
          'tracker/'
          'trading/'
          'traf/'
          'traffic'
          'traffic/'
          'traffix/'
          'trainersctw/'
          'transaction'
          'transactions'
          'transfer'
          'transit/'
          'transition/'
          'translate/'
          'transport'
          'trap'
          'trash'
          'trash/'
          'travelscape/'
          'tree'
          'tree/'
          'trees'
          'trends/'
          'trendsmusic/'
          'trforen/'
          'trial'
          'trial/'
          'triang/'
          'trier'
          'trthemenforen/'
          'truncate'
          'truncate/'
          'trust/'
          'tst'
          'tsts'
          'tsweb/'
          'tub/'
          'tully/'
          'tutorial'
          'tutorial/'
          'tv/'
          'twiki-bin/'
          'two'
          'two/'
          'txt-files/'
          'typo3/'
          'u'
          'u/'
          'u_name'
          'uat/'
          'uddi'
          'uddi/'
          'uddipublic/'
          'uds/'
          'ugcauthor/'
          'ugccategory/'
          'uid'
          'uin.txt'
          'uin/'
          'uins.dat'
          'uins.php'
          'uins.txt'
          'uins/'
          'uk'
          'uk/'
          'ultra/'
          'ultrasearch/'
          'un'
          'uname'
          'unc/'
          'unclesam/'
          'uni'
          'uni/'
          'uninstall'
          'uninstall.php'
          'uninstall/'
          'univ/'
          'unix'
          'unix/'
          'unsubscribe/'
          'up'
          'up/'
          'update'
          'update.php'
          'update/'
          'updatedesign/'
          'updates'
          'updates/'
          'upfile/'
          'upgrade.txt'
          'upgrade/'
          'upl'
          'upl.php'
          'uplfile/'
          'upload'
          'upload.php'
          'upload/'
          'uploaded/'
          'uploader'
          'uploadfile.php'
          'uploadfiles.php'
          'uploads'
          'uploads.php'
          'uploads/'
          'uploadz'
          'ups/'
          'ur-admin.%EXT%'
          'ur-admin.asp'
          'ur-admin.html'
          'ur-admin.php'
          'ur-admin/'
          'urchin'
          'urchin/'
          'url/'
          'urls/'
          'urltrurl/'
          'us'
          'usage'
          'usage/'
          'usebb_members'
          'user'
          'user.%EXT%'
          'user/'
          'user/admin.%EXT%'
          'user1'
          'user_admin'
          'user_email'
          'user_files/'
          'user_icq'
          'user_id'
          'user_info'
          'user_ip'
          'user_level'
          'user_list'
          'user_login'
          'user_logins'
          'user_n'
          'user_name'
          'user_names'
          'user_pass'
          'user_passw'
          'user_passwd'
          'user_password'
          'user_pw'
          'user_pwd'
          'user_pword'
          'user_pwrd'
          'user_un'
          'user_uname'
          'user_username'
          'user_usernm'
          'user_usernun'
          'user_usrnm'
          'useradmin'
          'useradmin/'
          'usercontrol'
          'usercounter/'
          'userid'
          'userinfo'
          'userip'
          'userlist'
          'userlogin'
          'userlogin.%EXT%'
          'userlogins'
          'usern'
          'username'
          'usernames'
          'usernm'
          'userpass'
          'userpassword'
          'userpassword/'
          'userpw'
          'userpwd'
          'userregister/'
          'userrights'
          'users'
          'users.%EXT%'
          'users.inc'
          'users/'
          'users/admin.%EXT%'
          'ushomeland/'
          'usparkpolice/'
          'usr'
          'usr/'
          'usr2'
          'usr_n'
          'usr_pass'
          'usrn'
          'usrnam'
          'usrname'
          'usrnm'
          'usrpass'
          'usrs'
          'ustats'
          'util'
          'util/'
          'utilisateur'
          'utilisateurs'
          'utilites/'
          'utilities'
          'utilities/'
          'utility'
          'utility/'
          'utility_login/'
          'utils'
          'utils/'
          'utlisateur'
          'utvutst/'
          'uvpanel/'
          'uvw_Category'
          'uvw_Pref'
          'uvw_Preferences'
          'v'
          'v/'
          'v2/painel/'
          'v4images/'
          'vadmind/'
          'valid'
          'validate'
          'validated'
          'validation'
          'validatior'
          'vap'
          'var'
          'var/'
          'vb'
          'vb_user'
          'vbs'
          'vbscript'
          'vbscripts'
          'vbulletin_session'
          'vbulletin_user'
          'vda/'
          'vdns/'
          'vehicleshipper/'
          'venonai/'
          'venues'
          'verity/'
          'version/'
          'vfs'
          'vgn/'
          'vh/'
          'video'
          'video.php'
          'video/'
          'videopreview/'
          'videoprogram/'
          'videos/'
          'videosearch/'
          'view'
          'view.php'
          'view/'
          'viewLogBookEntry'
          'viewStoneArtist'
          'viewer'
          'viewers/'
          'views'
          'views/'
          'vigilanceimages/'
          'virdiruniverse/'
          'virgin'
          'virtual'
          'visacard/'
          'visit/'
          'visitor'
          'vmailadmin/'
          'vmware'
          'vmware/'
          'vod/'
          'voip'
          'voip/'
          'voodoo_members'
          'vorod.%EXT%'
          'vorod/'
          'vorud.%EXT%'
          'vorud/'
          'vote/'
          'votes.php'
          'vpn'
          'vpn/'
          'vrtx/'
          'vtadmin/'
          'vti/maildir'
          'vti_cnf'
          'vti_cnf/'
          'vti_pvt/'
          'vwListAllAvailable'
          'w'
          'w/'
          'w3'
          'w3/'
          'w3c'
          'w3c/'
          'wallet/'
          'wan'
          'wan/'
          'wanted/'
          'wap'
          'wap/'
          'wapsearch/'
          'warez'
          'warez.html'
          'warez/'
          'wasecure/'
          'wb/'
          'wdav'
          'weather/'
          'weatherbroad/'
          'web'
          'web/'
          'web/phpMyAdmin/'
          'web_admin/'
          'web_administrator/'
          'web_store.cgi'
          'webaccess'
          'webaccess/'
          'webad/'
          'webadmin'
          'webadmin.%EXT%'
          'webadmin.asp'
          'webadmin.html'
          'webadmin.nsf'
          'webadmin.php'
          'webadmin/'
          'webadministrator/'
          'webadmins'
          'webalizer'
          'webalizer/'
          'webapp'
          'webboard'
          'webboard/'
          'webcam'
          'webcam/'
          'webcart'
          'webcash/'
          'webcerts/'
          'webcgi/'
          'webchannels/'
          'webdata'
          'webdav'
          'webdist'
          'webgalerie1/'
          'webhits'
          'webhosting/'
          'webicons/'
          'webimages/'
          'weblink/'
          'weblog'
          'weblog/'
          'weblogic'
          'weblogs'
          'weblogs/'
          'webmail'
          'webmail/'
          'webmail_notify/'
          'webmailtext/'
          'webmaster'
          'webmaster.%EXT%'
          'webmaster/'
          'webmasterlogs/'
          'webmasters'
          'webmasters/'
          'webmasterstats/'
          'webperl/'
          'websearch'
          'webservice'
          'webservices'
          'webshop/'
          'website'
          'website/'
          'websiteimages/'
          'websites/'
          'websitesize/'
          'websitetools/'
          'webssi/'
          'webstat'
          'webstats'
          'webstats/'
          'webstatsberkmapper/'
          'websvn/'
          'websync/'
          'webtest/'
          'webtools/'
          'webtour/'
          'webtrends/'
          'webuser'
          'webusers'
          'webusers/'
          'webvpn'
          'web~/'
          'welcome'
          'welcome/'
          'welcomekit/'
          'wellcome'
          'wgbh/'
          'what/'
          'whatever'
          'whatnew/'
          'whatnot'
          'whats/'
          'whatsnewtext/'
          'whmotext/'
          'whois'
          'whquiztext/'
          'whyregister.php'
          'wi/'
          'wiki/'
          'wiki/pre'
          'wikipedia'
          'wikipedia/'
          'wikipub/'
          'wikiuploads/'
          'will'
          'willopen/'
          'win'
          'win-c-sample.exe'
          'win/'
          'windows'
          'windows/'
          'windowsmobile/'
          'winhec/'
          'winnt'
          'winnt/'
          'wireless'
          'wireless/'
          'wireless1/'
          'wishesgranted/'
          'wiss/'
          'wissvictoria/'
          'wizard/'
          'wizards/'
          'wizmysqladmin/'
          'wm/'
          'wmail/'
          'wmdtext/'
          'wml/'
          'wmlsearch/'
          'wmr/'
          'wmv'
          'wmv/'
          'wmz/'
          'wnu'
          'womenimages/'
          'word'
          'work'
          'work.html'
          'work/'
          'workfiles/'
          'working'
          'working/'
          'workplace'
          'workshop'
          'workshop/'
          'wp-admin'
          'wp-admin/'
          'wp-content/'
          'wp-login.php'
          'wp-login/'
          'wp_users'
          'wr/'
          'write'
          'write/'
          'ws'
          'ws/'
          'ws_ftp'
          'ws_ftp/'
          'wstats'
          'wsu/'
          'wsuarchive/'
          'wusage'
          'wusage/'
          'ww2/'
          'www'
          'www-archive/'
          'www2/'
          'wwwboard'
          'wwwboard/'
          'wwwboardeleeteleetwwwclass/'
          'wwwcore/'
          'wwwjoin'
          'wwwjoin/'
          'wwwlog'
          'wwwlog/'
          'wwwroot/'
          'wwwstats'
          'wyflyfish/'
          'x'
          'x.php'
          'x.txt'
          'x/'
          'x0'
          'x0/'
          'x1'
          'x1/'
          'x2'
          'x2/'
          'x_admin'
          'xak/'
          'xar_name'
          'xar_pass'
          'xar_roles'
          'xcache'
          'xfer'
          'xfile/'
          'xhtml/'
          'xlogin/'
          'xls/'
          'xml'
          'xml/'
          'xmlin/'
          'xmlout/'
          'xmlrpc'
          'xoops_bannerclient'
          'xoops_users'
          'xp'
          'xp/'
          'xsl'
          'xsql'
          'xx'
          'xx.php'
          'xx/'
          'xxx'
          'xxx.php'
          'xxx/'
          'xxxx'
          'xxxx/'
          'xxxxx'
          'xxxxx/'
          'xxxxxx'
          'xxxxxx/'
          'xxxxxxx'
          'xxxxxxx/'
          'xxxxxxxx'
          'xxxxxxxx/'
          'xxxxxxxxx'
          'xxxxxxxxx/'
          'xxxxxxxxxx'
          'xxxxxxxxxx/'
          'xy'
          'xy/'
          'xyz'
          'xyz/'
          'xyza/'
          'y'
          'y/'
          'yabb_settings'
          'yabbse_settings'
          'yahoosearch/'
          'year/'
          'yijian/'
          'yonetici.%EXT%'
          'yonetici.asp'
          'yonetici.html'
          'yonetici.php'
          'yonetim.%EXT%'
          'yonetim.asp'
          'yonetim.html'
          'yonetim.php'
          'z'
          'z/'
          'z_dx/'
          'zap'
          'zdnetdownloads/'
          'zero'
          'zero/'
          'zerod')
        ReadOnly = True
        ScrollBars = ssVertical
        TabOrder = 0
      end
    end
    object OkTab: TTabSheet
      Caption = '200 [OK]'
      ImageIndex = 1
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Memo2: TMemo
        Left = 3
        Top = 3
        Width = 345
        Height = 187
        Color = 52224
        ScrollBars = ssVertical
        TabOrder = 0
      end
    end
    object FoundTab: TTabSheet
      Caption = '302 [FOUND]'
      ImageIndex = 2
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Memo3: TMemo
        Left = 3
        Top = 3
        Width = 345
        Height = 187
        Color = 6599167
        ScrollBars = ssVertical
        TabOrder = 0
      end
    end
    object ForbiddenTab: TTabSheet
      Caption = '403 [FORBIDDEN]'
      ImageIndex = 3
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Memo4: TMemo
        Left = 3
        Top = 3
        Width = 345
        Height = 187
        Color = 5987327
        ScrollBars = ssVertical
        TabOrder = 0
      end
    end
  end
  object StatBox: TGroupBox
    Left = 8
    Top = 226
    Width = 361
    Height = 55
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 1
    object Gauge1: TGauge
      Left = 7
      Top = 31
      Width = 345
      Height = 16
      ForeColor = 15329769
      Progress = 0
    end
    object Edit1: TEdit
      Left = 7
      Top = 8
      Width = 186
      Height = 17
      Color = 16579836
      TabOrder = 0
      Text = 'http://mavashimisha.ru/'
    end
    object GroupBox1: TGroupBox
      Left = 199
      Top = 8
      Width = 153
      Height = 17
      Color = clWhite
      Ctl3D = False
      ParentBackground = False
      ParentColor = False
      ParentCtl3D = False
      TabOrder = 1
      object Label1: TLabel
        Left = 11
        Top = 3
        Width = 46
        Height = 11
        Caption = 'checked: '
      end
      object Label2: TLabel
        Left = 74
        Top = 3
        Width = 7
        Height = 11
        Caption = '0'
      end
    end
  end
  object IdHTTP1: TIdHTTP
    IOHandler = IdSSLIOHandlerSocketOpenSSL1
    AllowCookies = True
    ProxyParams.BasicAuthentication = False
    ProxyParams.ProxyPort = 0
    Request.ContentEncoding = 'windows-1251'
    Request.ContentLength = -1
    Request.ContentRangeEnd = -1
    Request.ContentRangeStart = -1
    Request.ContentRangeInstanceLength = -1
    Request.Accept = 'text/html, */*'
    Request.BasicAuthentication = False
    Request.UserAgent = 'Mozilla/3.0 (compatible; Indy Library)'
    Request.Ranges.Units = 'bytes'
    Request.Ranges = <>
    HTTPOptions = [hoForceEncodeParams]
    Left = 144
    Top = 57
  end
  object Menu: TMainMenu
    Left = 200
    Top = 56
    object Action1: TMenuItem
      Caption = 'Action'
      object Start1: TMenuItem
        Caption = 'Start'
        OnClick = Start1Click
      end
      object Pause1: TMenuItem
        Caption = 'Pause'
        Enabled = False
        OnClick = Pause1Click
      end
      object Resume1: TMenuItem
        Caption = 'Resume'
        Enabled = False
        OnClick = Resume1Click
      end
      object Stop1: TMenuItem
        Caption = 'Stop'
        Enabled = False
        OnClick = Stop1Click
      end
      object Exit1: TMenuItem
        Caption = 'Exit'
        OnClick = Exit1Click
      end
    end
    object About1: TMenuItem
      Caption = 'About'
      OnClick = About1Click
    end
  end
  object IdSSLIOHandlerSocketOpenSSL1: TIdSSLIOHandlerSocketOpenSSL
    MaxLineAction = maException
    Port = 0
    DefaultPort = 0
    SSLOptions.Mode = sslmUnassigned
    SSLOptions.VerifyMode = []
    SSLOptions.VerifyDepth = 0
    Left = 176
    Top = 120
  end
  object HTTP: TNetHTTPClient
    AllowCookies = True
    HandleRedirects = True
    UserAgent = 'Embarcadero URI Client/1.0'
    Left = 264
    Top = 56
  end
end
